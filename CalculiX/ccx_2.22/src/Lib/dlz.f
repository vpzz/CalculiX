c
c Module 496 in TOMS
c Based upon the LZ-algorithm
c (see L.C. Kaufman, ACM TOMS 1 (1975) pp. 271-281)
c
...
C WHEN THE DENOMINATOR IS SMALL
        Y = DCMPLX(DREAL(W)/D1,DIMAG(W)/D1)/DCMPLX(DREAL(Z)/D1,DIMAG(Z)/
     *   D1)
        GO TO 200
  190   Y = DCMPLX(DREAL(Z)/D2,DIMAG(Z)/D2)/DCMPLX(DREAL(W)/D2,DIMAG(W)/
     *   D2)
...
C MUST INTERCHANGE COLUMNS
  ...
  260   IF (D2.EQ.0.D0) GO TO 310
        Z = DCMPLX(DREAL(W)/D1,DIMAG(W)/D1)/DCMPLX(DREAL(Z)/D1,DIMAG(Z)/
     *   D1)
        GO TO 280
  270   Z = DCMPLX(DREAL(Z)/D2,DIMAG(Z)/D2)/DCMPLX(DREAL(W)/D2,DIMAG(W)/
     *   D2)
  280   DO 290 K=LOR1,J
          A(K,I) = A(K,I) - Z*A(K,J)
          B(K,I) = B(K,I) - Z*B(K,J)
