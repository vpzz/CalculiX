...
void utempread(double *t1, ITG *istep, char *jobnamec) {
  ...
  if ((anz[0].n == 0) || (anz[0].e == 0)) {
    printf(
        " *ERROR in utempread: there are either no nodes or\n no elements or "
        "neither nodes nor elements in the temperature frd-file\n");
    FORTRAN(stop, ());
  }
  ...
  if (loadcase > -1) {
    ...
    printf(
        "INFO in utempread: no temperature data\n was found for step %d in the "
        "temperature frd-file\n\n",
        *istep);
  }
