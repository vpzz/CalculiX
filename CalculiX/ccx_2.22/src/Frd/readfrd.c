...
/* in any case for each results of a new step the first data-block is readed to
 * see how much nodes are included */

int readfrd(char *datin, Summen *anz, Nodes **nptr, Elements **eptr,
            Datasets **lptr, int read_mode) {
  FILE *handle;
  int i = 0, j = 0;
  ...
  int elem_data = 0, nod_data = 0,
      nod_1st_block = 0; /* nodes in resultblock, nodes in 1st block (if no "nr
                            of nodes" are given in frd file, 100C-line) */
  ...
  if (handle == NULL) {
    printf("ERROR in readfrd: The input file \"%s\" could not be opened.\n\n",
           datin);
    return (-1);
  } else
  ...
  while (length) {
    ...
    if (filepntr == NULL) {
      if ((filepntr = (fpos_t *)malloc(1 * sizeof(fpos_t))) == NULL)
        printf(" ERROR: malloc failed\n");
    }

    if (fgetpos(handle, (fpos_t *)filepntr) != 0) {
  ...
  read_again:;
    ...
    if (flag == 1) {
      ...
      if (rec_str[5] == 'U') {
        ...
        if (!anz->u) {
          if ((anz->uheader = (char **)malloc(sizeof(char *))) == NULL)
            printf("\n\n ERROR: malloc failed\n\n");
        } else if ((anz->uheader = (char **)realloc(
                        (char **)anz->uheader,
                        (anz->u + 1) * sizeof(char *))) == NULL)
          printf("\n\n ERROR: realloc failed\n\n");
        if ((anz->uheader[anz->u] =
                 (char *)malloc(MAX_LINE_LENGTH * sizeof(char))) == NULL)
          printf("\n\n ERROR: malloc failed\n\n");
      ...
      if (rec_str[5] == 'P') {
        ...
        if (!anz_p) {
          if ((pheader = (char **)malloc(sizeof(char *))) == NULL)
            printf("\n\n ERROR: malloc failed\n\n");
        } else if ((pheader = (char **)realloc((char **)pheader,
                                               (anz_p + 1) * sizeof(char *))) ==
                   NULL)
          printf("\n\n ERROR: realloc failed\n\n");
        if ((pheader[anz_p] = (char *)malloc(MAX_LINE_LENGTH * sizeof(char))) ==
            NULL)
          printf("\n\n ERROR: malloc failed\n\n");
      ...
      /* nr of nodes per block can be read from the frd file, this is not
       * documented in the original frd-spec. */
      nod_data = stoi(rec_str, 25, 36);
      if (nod_data > 0)
        node_field_size = nod_data;
      else
        node_field_size = INI_FIELD_SIZE;
      do {
        if ((node = (Nodes *)realloc((Nodes *)node,
                                     (node_field_size + 1) * sizeof(Nodes))) ==
            NULL) {
          printf(
              "WARNING: in readfrd() is INI_FIELD_SIZE:%d to large and is "
              "reduced\n",
              node_field_size);
          node_field_size /= 2;
        }
      ...
      if (format_flag < 2) {
        do {
          ...
          if (flag == -3) break;
          if (!format_flag)
            node[anz->n].nr = stoi(rec_str, 4, 8);
          else
            node[anz->n].nr = stoi(rec_str, 4, 13);
          if (node[anz->n].nr >= node_field_size) {
            if (node[anz->n].nr < MAX_INTEGER / 2)
              node_field_size = node[anz->n].nr * 2 + 1;
            else
              node_field_size = MAX_INTEGER - 2;
            nodenr = node[anz->n].nr;
            do {
              if ((node = (Nodes *)realloc(
                       (Nodes *)node, (node_field_size + 1) * sizeof(Nodes))) ==
                  NULL) {
                printf(
                    "WARNING: in readfrd() is INI_FIELD_SIZE:%d to large and "
                    "is reduced\n",
                    node_field_size);
              ...
              if (node_field_size <= nodenr) {
                printf(
                    "\n\n ERROR: not enough memory in readfrd() for node-nr:%d "
                    "available\n\n",
                    nodenr);
                exit(-1);
              }
      ...
      /* binary format */
        ...
        if ((value = (float *)realloc((float *)value, (3) * sizeof(float))) ==
            NULL)
        ...
        for (i = 0; i < nod_data; i++) {
          ...
          if (node[anz->n].nr >= node_field_size) {
            ...
            do {
              ...
              if (node_field_size <= nodenr) {
                printf(
                    "\n\n ERROR: not enough memory in readfrd() for the "
                    "node-nr:%d available\n\n",
                    nodenr);
                exit(-1);
              }
          ...
          if (format_flag == 2) {
            ...
            if (node[node[anz->n].nr].indx < 0) {
              ...
              fseeko(handle, 3 * sizeof(float), SEEK_CUR);
          } else {
            if (node[node[anz->n].nr].indx < 0) {
              length = fread((double *)&node[node[anz->n].nr].nx,
                             sizeof(double), 3, handle);
              node[node[anz->n].nr].indx = anz->n;
            } else
              fseeko(handle, 3 * sizeof(double), SEEK_CUR);
          }
          if (node[anz->n].nr > anz->nmax) anz->nmax = node[anz->n].nr;
          if (node[anz->n].nr < anz->nmin) anz->nmin = node[anz->n].nr;
#if TEST
      ...
      if ((node = (Nodes *)realloc((Nodes *)node,
                                   node_field_size * sizeof(Nodes))) == NULL)
        printf("\n\n ERROR: realloc failed\n\n");
      else
        printf("\n %d nodes reallocated \n", anz->nmax);
      // nod_1st_block=anz->n;
      ...
      /* nr of elems per block can be read from the frd file, this is not
       * documented in the original frd-spec. */
      elem_data = stoi(rec_str, 25, 36);
      if (elem_data > 0)
        elem_field_size = elem_data;
      else
        elem_field_size = INI_FIELD_SIZE;
      do {
        if ((elem = (Elements *)realloc(
                 (Elements *)elem, (elem_field_size + 1) * sizeof(Elements))) ==
            NULL) {
          printf(
              "WARNING: in readfrd() is INI_FIELD_SIZE:%d to large and is "
              "reduced\n",
      ...
      /* binary format */
      if (format_flag == 2) {
        if ((elem = (Elements *)realloc((Elements *)elem,
                                        elem_data * sizeof(Elements))) == NULL)
        ...
        for (i = 0; i < elem_data; i++) {
          ...
          if (elem[anz->e].type == 1)
            ipuf = 8; /* HEXA8  */
          else if (elem[anz->e].type == 2)
            ipuf = 6; /* PE6   */
          else if (elem[anz->e].type == 3)
            ipuf = 4; /* TET4   */
          else if (elem[anz->e].type == 4)
            ipuf = 20; /* HEXA20 */
          else if (elem[anz->e].type == 5)
            ipuf = 15; /* PE15  */
          else if (elem[anz->e].type == 6)
            ipuf = 10; /* TET10  */
          else if (elem[anz->e].type == 7)
            ipuf = 3; /* TRI3   */
          else if (elem[anz->e].type == 8)
            ipuf = 6; /* TRI6   */
          else if (elem[anz->e].type == 9)
            ipuf = 4; /* QUAD4  */
          else if (elem[anz->e].type == 10)
            ipuf = 8; /* QUAD8  */
          else if (elem[anz->e].type == 11)
            ipuf = 2; /* BEAM2   */
          else if (elem[anz->e].type == 12)
            ipuf = 3; /* BEAM3   */
                      // printf("el:%d t:%d g:%d m:%d n:%d\n", elem[anz->e].nr,
          // elem[anz->e].type,elem[anz->e].group,elem[anz->e].mat, ipuf);
          length = fread((int *)elem[anz->e].nod, sizeof(int), ipuf, handle);
          // for(j=0;j<ipuf; j++) printf(" %d",elem[anz->e].nod[j]);
          // printf("\n");
        }
        anz->e++;
      } else {
        do {
          ...
          if (flag == -3)
            ...
            if (anz->e >= elem_field_size) {
              if (anz->e < MAX_INTEGER / 2)
                elem_field_size = anz->e * 2 + 1;
              else
                elem_field_size = MAX_INTEGER - 2;
              do {
                if ((elem = (Elements *)realloc(
                         (Elements *)elem,
                         (elem_field_size + 1) * sizeof(Elements))) == NULL) {
                  printf(
                      "WARNING: in readfrd() is INI_FIELD_SIZE:%d to large and "
                      "is reduced\n",
                      elem_field_size);
              ...
              if (!format_flag) {
                ...
                for (i = 0; i < ipuf; i++) {
                  ...
                  if (j < 14)
                    ...
                    if (i < ipuf - 1) length = frecord(handle, rec_str);
                    j = 0;
                  }
                }
              } else {
        ...
        if ((elem = (Elements *)realloc(
                 (Elements *)elem, elem_field_size * sizeof(Elements))) == NULL)
          printf("\n\n ERROR: in readfrd realloc failed\n\n");
        else
      ...
      if ((lcase = (Datasets *)realloc(
               (Datasets *)lcase, (anz->l + 2) * sizeof(Datasets))) == NULL) {
       ...
       * documented in the original frd-spec. */
      nod_data = stoi(rec_str, 25, 36);
      /* because of a bug in ccx2.0 this nr can be wrong. In this case it is
       * higher than the actual nr of nodes. */
      if (nod_data > anz->n) {
        printf(
            " WARNING: in this result-block are more nodes announced:%d than "
            "in the model defined:%d\n Please inform the program-admin of the "
            "originator of the frd-file\n\n",
            nod_data, anz->n);
        // exit(0);
        nod_data = anz->n;
      }
      ...
      ipuf = -1;
      // if(lcase[anz->l].analysis_type==2) //in the moment ccx writes the wrong
      // number, therefore:
        ...
        if (ipuf != -1)
          for (i = 0; i < lcase[anz->l].npheader; i++) {
            if (compare(&lcase[anz->l].pheader[i][5], "PMODE", 5) == 5) {
              ...
              if (ipuf > -1)
                sprintf(&lcase[anz->l]
                             .dataset_text[strlen(lcase[anz->l].dataset_text)],
                        " MODE:%d", ipuf);
            }
          }
      }
      ...
      do {
        /* bin mode, active after last column definition was read (-5 lines).
         * Attention flag:-6 not permitted so far! */
        if ((format_flag >= 2) && (lcase[anz->l].ncomps > 0) &&
            (ncomps == lcase[anz->l].ncomps)) {
          // printf("format_flag=%d ncomps:%d
          // lcncomps:%d\n",format_flag,ncomps,lcase[anz->l].ncomps);

          /* if offset is known jump the filepointer before the next block or
           * else continue reading assuming values for all nodes are provided */
          if (offset) {
            ...
            if (firsttime) {
              ...
              if (fseeko(handle, offset, SEEK_CUR) != 0)
                printf("error in fseeko\n");
            }
          } else if (format_flag == 2) {
            if ((value = (float *)realloc(
                     (float *)value, (lcase[anz->l].ncomps) * sizeof(float))) ==
                NULL)
              printf("\n\n ERROR: realloc failed, value\n\n");
            for (n = 0; n < nod_data; n++) {
              length = fread((int *)&nodenr, sizeof(int), 1, handle);
              length = fread((float *)value, sizeof(float),
                             lcase[anz->l].ncomps, handle);
            ...
            if ((dvalue = (double *)realloc(
                     (double *)dvalue,
                     (lcase[anz->l].ncomps) * sizeof(double))) == NULL)
              printf("\n\n ERROR: realloc failed, dvalue\n\n");
            for (n = 0; n < nod_data; n++) {
              length = fread((int *)&nodenr, sizeof(int), 1, handle);
              length = fread((double *)dvalue, sizeof(double),
                             lcase[anz->l].ncomps, handle);
              // printf("n:%d N:%d ",n+1, nodenr);
              for (i = 0; i < lcase[anz->l].ncomps; i++) {
                // printf(" %f",dvalue[i]);
        ...
        if (flag == -1) {
          /* if offset is known jump the filepointer before the next block and
           * continue reading until flag=-3  */
          if (offset) {
            lcase[anz->l].loaded = 0;
            if (firsttime) {
              firsttime = 0;
              ...
              if (fseeko(handle, offset - length, SEEK_CUR) != 0)
                printf("error in fseeko\n");
            }
          } else {
            nod_1st_block++;
            if (format_flag)
              nodenr = stoi(rec_str, 4, 13);
            else
              nodenr = stoi(rec_str, 4, 8);
            if (nodenr > anz->nmax) {
              if (!errFlag) {
                errFlag = 1;
                printf(
                    "WARNING: found node:%d in Dataset higher than in geometry "
                    "allocated:%d\n",
                    nodenr, anz->nmax);
              }
              ...
              if (maxcomps == 6) {
                ...
                for (i = 0; i < maxcomps; i++)
                  lcase[anz->l].dat[i][nodenr] =
                      stof(&rec_str[n + i * 12], 1, 12);
              }
              /* printf("%d", nodenr); for (i=0; i<maxcomps; i++) printf("
               * %f",lcase[anz->l].dat[i][nodenr] ); printf("\n"); */
            } else
              i = 0;
          }
        } else if (flag == -2) {
          ...
          if ((lcase[anz->l].nmax =
                   (int *)malloc((lcase[anz->l].ncomps) * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].nmin =
                   (int *)malloc((lcase[anz->l].ncomps) * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].max = (CGXFLOAT *)malloc(
                   (lcase[anz->l].ncomps) * sizeof(CGXFLOAT))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].min = (CGXFLOAT *)malloc(
                   (lcase[anz->l].ncomps) * sizeof(CGXFLOAT))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].compName = (char **)malloc(
                   (lcase[anz->l].ncomps) * sizeof(char *))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].icname = (char **)malloc((lcase[anz->l].ncomps) *
                                                      sizeof(char *))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].menu =
                   (int *)malloc(lcase[anz->l].ncomps * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].ictype =
                   (int *)malloc(lcase[anz->l].ncomps * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].icind1 =
                   (int *)malloc(lcase[anz->l].ncomps * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].icind2 =
                   (int *)malloc(lcase[anz->l].ncomps * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].iexist =
                   (int *)malloc(lcase[anz->l].ncomps * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          if ((lcase[anz->l].dat = (CGXFLOAT **)malloc(
                   (lcase[anz->l].ncomps) * sizeof(CGXFLOAT *))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          printf(" gen lc[%d] ncomps:%d\n", anz->l, lcase[anz->l].ncomps);
          for (i = 0; i < (lcase[anz->l].ncomps); i++) {
            if ((lcase[anz->l].compName[i] =
                     (char *)malloc(MAX_LINE_LENGTH * sizeof(char))) == NULL)
              printf("\n\n ERROR: malloc failed\n\n");
            if ((lcase[anz->l].icname[i] =
                     (char *)malloc(MAX_LINE_LENGTH * sizeof(char))) == NULL)
              printf("\n\n ERROR: malloc failed\n\n");
          ...
          if (indx < lcase[anz->l].ncomps) {
            ...
            if (lcase[anz->l].iexist[indx] == 1) {
              SFREE(lcase[anz->l].compName[indx]);
              lcase[anz->l].compName[indx] = NULL;
              SFREE(lcase[anz->l].icname[indx]);
              lcase[anz->l].icname[indx] = NULL;
            } else
              ncomps++;
            indx++;
          } else {
            ...
            printf(" WARNING: unallocated component:%d \"%s\" %d\n", ncomps,
                   &rec_str[5], lcase[anz->l].ncomps);
            exit(0);
          }
          nentities++;

          /* this is the last -5 line, try to figure out an offset (length of
           * data-block) for the file-pointer */
          /* and allocate data if no offset is defined (first time) */
          if (nentities == lcase[anz->l].ncomps) {
            lcase[anz->l].ncomps = ncomps;
            if (lcase[anz->l].ncomps < 6)
              maxcomps = lcase[anz->l].ncomps;
            else
              maxcomps = 6;

            nvals = 0;
            for (i = 0; i < ncomps; i++)
              if (lcase[anz->l].iexist[i] != 1) nvals++;
            ...
            if (!offset) {
              /* in case the data should be read directly and not on demand */
              for (i = 0; i < (lcase[anz->l].ncomps); i++) {
                if ((lcase[anz->l].dat[i] = (CGXFLOAT *)malloc(
                         (anz->nmax + 1) * sizeof(CGXFLOAT))) == NULL)
                  printf("\n\n ERROR: malloc failure\n\n");
          ...
          for (i = 0; i < ipuf; i++)
            strcpy(compName[i], lcase[anz->l].compName[atoi(dat[i + 3]) - 1]);
          for (i = 0; i < ipuf; i++)
            strcpy(lcase[anz->l].compName[i], compName[i]);
          ...
          if ((iexist = (int *)malloc(ipuf * sizeof(int))) == NULL)
            printf("\n\n ERROR: malloc failure\n\n");
          for (i = 0; i < ipuf; i++)
            menu[i] = lcase[anz->l].menu[atoi(dat[i + 3]) - 1];
          for (i = 0; i < ipuf; i++) lcase[anz->l].menu[i] = menu[i];
          for (i = 0; i < ipuf; i++)
            ictype[i] = lcase[anz->l].ictype[atoi(dat[i + 3]) - 1];
          for (i = 0; i < ipuf; i++) lcase[anz->l].ictype[i] = ictype[i];
          for (i = 0; i < ipuf; i++)
            icind1[i] = lcase[anz->l].icind1[atoi(dat[i + 3]) - 1];
          for (i = 0; i < ipuf; i++) lcase[anz->l].icind1[i] = icind1[i];
          for (i = 0; i < ipuf; i++)
            icind2[i] = lcase[anz->l].icind2[atoi(dat[i + 3]) - 1];
          for (i = 0; i < ipuf; i++) lcase[anz->l].icind2[i] = icind2[i];
          for (i = 0; i < ipuf; i++)
            iexist[i] = lcase[anz->l].iexist[atoi(dat[i + 3]) - 1];
      ...
      if ((lcase[anz->l].nmin =
               (int *)malloc(lcase[anz->l].ncomps * sizeof(int))) == NULL)
        printf("\n\n ERROR: malloc failure\n\n");
      if ((lcase[anz->l].max = (CGXFLOAT *)malloc(lcase[anz->l].ncomps *
                                                  sizeof(CGXFLOAT))) == NULL)
        printf("\n\n ERROR: malloc failure\n\n");
      if ((lcase[anz->l].min = (CGXFLOAT *)malloc(lcase[anz->l].ncomps *
                                                  sizeof(CGXFLOAT))) == NULL)
        printf("\n\n ERROR: malloc failure\n\n");
      if ((lcase[anz->l].dat = (CGXFLOAT **)malloc(lcase[anz->l].ncomps *
                                                   sizeof(CGXFLOAT *))) == NULL)
        printf("\n\n ERROR: malloc failure\n\n");
      if ((lcase[anz->l].compName =
               (char **)malloc(lcase[anz->l].ncomps * sizeof(char *))) == NULL)
        printf("\n\n ERROR: malloc failure\n\n");
      if ((lcase[anz->l].icname =
               (char **)malloc(lcase[anz->l].ncomps * sizeof(char *))) == NULL)
        printf("\n\n ERROR: malloc failure\n\n");
      for (i = 0; i < lcase[anz->l].ncomps; i++) {
        if ((lcase[anz->l].compName[i] =
    ...
    next:;
      ...
      printf("scip %d nodes\n", nod_data);
      if (format_flag == 2)
        fseeko(handle, nod_data * (sizeof(int) + 3 * sizeof(float)), SEEK_CUR);
      else
        fseeko(handle, nod_data * (sizeof(int) + 3 * sizeof(double)), SEEK_CUR);
    } else if (flag == -3) {
      /* nr of elems per block can be read from the frd file, this is not
       * documented in the original frd-spec. */
      elem_data = stoi(rec_str, 25, 36);
  ...
  for (i = 0; i < anz_p; i++) SFREE(pheader[i]);
  SFREE(pheader);
  SFREE(filepntr);
...
int readfrdblock(int lc, Summen *anz, Nodes *node, Datasets *lcase) {
  ...
  if (handle == NULL) {
    printf(
        "ERROR in readfrdblock: The input file \"%s\" could not be opened.\n\n",
        lcase[lc].filename);
    return (-1);
  }

  if (fsetpos(handle, (fpos_t *)lcase[lc].fileptr) != 0) {
  ...
  if ((lcase[lc].dat = (CGXFLOAT **)malloc((lcase[lc].ncomps) *
                                           sizeof(CGXFLOAT *))) == NULL)
    printf("\n\n ERROR: malloc failure\n\n");
  for (i = 0; i < (lcase[lc].ncomps); i++) {
    if ((lcase[lc].dat[i] =
             (CGXFLOAT *)malloc((anz->nmax + 1) * sizeof(CGXFLOAT))) == NULL)
      printf("\n\n ERROR: malloc failure\n\n");
  ...
  if (flag == 100) {
    ...
    do {
      ...
      if (format_flag >= 2) {
        ...
        if (format_flag == 2) {
          if ((value = (float *)realloc(
                   (float *)value, (lcase[lc].ncomps) * sizeof(float))) == NULL)
            printf("\n\n ERROR: realloc failed, value\n\n");
          for (n = 0; n < nod_data; n++) {
            length = fread((int *)&nodenr, sizeof(int), 1, handle);
            length =
                fread((float *)value, sizeof(float), lcase[lc].ncomps, handle);
            // printf("N:%d ",nodenr);
            for (i = 0; i < lcase[lc].ncomps; i++) {
              // printf(" %f",value[i]);
        ...
        if (format_flag == 3) {
          if ((dvalue = (double *)realloc(
                   (double *)dvalue, (lcase[lc].ncomps) * sizeof(double))) ==
              NULL)
            printf("\n\n ERROR: realloc failed, dvalue\n\n");
          for (n = 0; n < nod_data; n++) {
            length = fread((int *)&nodenr, sizeof(int), 1, handle);
            length = fread((double *)dvalue, sizeof(double), lcase[lc].ncomps,
                           handle);
            // printf("N:%d ",nodenr);
            for (i = 0; i < lcase[lc].ncomps; i++) {
              // printf(" %f",dvalue[i]);
        ...
        if (nodenr > anz->nmax) {
          ...
          if (maxcomps == 6) {
            ...
            for (i = 0; i < maxcomps; i++)
              lcase[lc].dat[i][nodenr] = stof(&rec_str[n + i * 12], 1, 12);
        } else
          i = 0;
      } else if (flag == -2) {
        if (!format_flag)
...
int readOneNode(int lc, Summen *anz, Datasets *lcase, int nodenr, double **vptr,
                long *byte_offset) {
  ...
  if (handle == NULL) {
    printf(
        "ERROR in readOneNode: The input file \"%s\" could not be opened.\n\n",
        lcase[lc].filename);
    return (-1);
  }
  ...
  /* the header-lines must be skipped in case byte_offset==0 and format ==
   * 2(bin) */
  if ((lcase[lc].format_flag >= 2) && (!*byte_offset)) {
    ...
    if (fseeko(handle, *byte_offset, SEEK_CUR) != 0)
      printf("error in fseeko\n");
  }
  ...
  *vptr = dat;
  ...
  if (lcase[lc].format_flag == 2) {
    if ((value = (float *)realloc((float *)value,
                                  (lcase[lc].ncomps) * sizeof(float))) == NULL)
      printf("\n\n ERROR: realloc failed, value\n\n");
    do {
  ...
  if (lcase[lc].format_flag == 3) {
    if ((dvalue = (double *)realloc(
             (double *)dvalue, (lcase[lc].ncomps) * sizeof(double))) == NULL)
      printf("\n\n ERROR: realloc failed, dvalue\n\n");
    do {
      length = fread((int *)&inodenr, sizeof(int), 1, handle) * sizeof(int);
      length +=
          fread((double *)dvalue, sizeof(double), lcase[lc].ncomps, handle) *
          sizeof(double);
      // printf("N:%d ",inodenr);
  ...
  do {
    ...
    if ((readFlag) && (flag == -2)) {
      ...
      if (!lcase[lc].format_flag)
        ...
        n = 13;
      if (nvals <= 6)
        offset += nod_data * (n + nvals * 12);
      else {
        for (i = 0; i < nvals / 6; i++) offset += nod_data * (n + 6 * 12 + 1);
        if (nvals % 6) offset += nod_data * (n + (nvals % 6) * 12 + 1);
      }
      *byte_offset = offset;
      ...
      if (lcase[lc].format_flag)
        inodenr = stoi(rec_str, 4, 13);
      else
        inodenr = stoi(rec_str, 4, 8);
      nod_data++;
      ...
      if (inodenr > anz->nmax) {
        ...
        if (maxcomps == 6) {
          ...
          for (i = 0; i < maxcomps; i++)
            dat[i] = stof(&rec_str[n + i * 12], 1, 12);
      } else
        i = 0;
    }
