...
void frdgeneralvector(double *v, ITG *iset, ITG *ntrans, char *filabl,
                      ...
                      char *m3, ITG *ioutall) {
  ITG i, j, k, l, nksegment, *inodeset = NULL, nnodeset, nnodeset_, *iy = NULL,
                             kflag;
    ...
    if ((*ntrans == 0) || (strcmp1(&filabl[5], "G") == 0)) {
      for (i = 0; i < *nkcoords; i++) {
        ...
        if (strcmp1(output, "asc") == 0) {
          if (mi[1] == 4) {
            fprintf(
                f1, "%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E%12.5E\n", m1, i + 1,
                (float)v[(mi[1] + 1) * i + 1], (float)v[(mi[1] + 1) * i + 2],
                (float)v[(mi[1] + 1) * i + 3], (float)v[(mi[1] + 1) * i + 4]);
          } else if (mi[1] == 5) {
            fprintf(
                f1, "%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E%12.5E%12.5E\n", m1,
                i + 1, (float)v[(mi[1] + 1) * i + 1],
                (float)v[(mi[1] + 1) * i + 2], (float)v[(mi[1] + 1) * i + 3],
                (float)v[(mi[1] + 1) * i + 4], (float)v[(mi[1] + 1) * i + 5]);
          } else if (mi[1] == 6) {
            fprintf(
                f1, "%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E%12.5E%12.5E%12.5E\n",
        ...
        if (inotr[2 * i] == 0) {
          if (strcmp1(output, "asc") == 0) {
            if (mi[1] == 4) {
              ...
              fprintf(
                  f1,
                  "%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E%12.5E%12.5E%12.5E\n",
                  m1, i + 1, (float)v[(mi[1] + 1) * i + 1],
                  (float)v[(mi[1] + 1) * i + 2], (float)v[(mi[1] + 1) * i + 3],
                  (float)v[(mi[1] + 1) * i + 4], (float)v[(mi[1] + 1) * i + 5],
                  (float)v[(mi[1] + 1) * i + 6]);
