...
void frd_sen(double *co, ITG *nk, double *dstn, ITG *inum, ITG *nmethod,
  ...
  if (*inorm == 1) {
    ...
    frdset(&filab[4002], set, &iset, istartset, iendset, ialset, inum, &noutloc,
    ...
    if (strcmp1(&filab[4002], "SEN") == 0) {
      ...
      frdset(&filab[4002], set, &iset, istartset, iendset, ialset, inum,
      ...
      frdselect(&v[2 * *nk * *iobject], v, &iset, &nkcoords, inum, m1,
                istartset, iendset, ialset, ngraph, &ncomptensord,
                ifieldtensord, icomptensord, nfieldtensord, &iselect, m2, f1,
                output, m3);
    }
    ...
    if ((strcmp1(&objectset[(*iobject - 1) * 405], "ALL-DISP") == 0) ||
      ...
      if (*noddiam >= 0) {
        frdheader(&icounter, &oner, time, &pi, noddiam, cs, &null, mode,
                  &noutloc, description, kode, nmethod, f1, output, istep,
                  iinc);
        ...
        frdvector(&v[*nk * mt], &iset, ntrans, &filab[4002], &nkcoords, inum,
                  m1, inotr, trab, co, istartset, iendset, ialset, mi, ngraph,
                  f1, output, m3, &ioutall);
      }
