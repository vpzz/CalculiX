...
static ITG *nobject1, *nk1, *nodedesi1, *ndesi1, *nx1, *ny1, *nz1, num_cpus,
    ifree1;
...
void transitionmain(double *co, double *feasdir, ITG *nobject, ITG *nk,
  ...
  if (strcmp1(&objectset[86], "BOU") == 0) {
    ...
    FORTRAN(pretransition,
            (ipkon, kon, lakon, co, nk, ipoface, nodface, nodedesiinv, xo, yo,
             zo, x, y, z, nx, ny, nz, &ifree));
    ...
    printf(" Using up to %" ITGFORMAT
           " cpu(s) for transition to sensitivities.\n\n",
           num_cpus);
...
void *transitionmt(ITG *i) {
  ...
  FORTRAN(transition,
          (feasdir1, nobject1, nk1, nodedesi1, ndesi1, objectset1, xo1, yo1,
           zo1, x1, yy1, z1, nx1, ny1, nz1, co1, &ifree1, &ndesia, &ndesib));
