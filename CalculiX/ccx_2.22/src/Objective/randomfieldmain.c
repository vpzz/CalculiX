...
void randomfieldmain(ITG *kon, ITG *ipkon, char *lakon, ITG *ne, ITG *nmpc,
  ...
  char bmat[2] = "I", which[3] = "SA", howmny[2] = "A", *objectset = NULL,
       *orname = NULL;

  ITG nzss, nzsd,
      *mast1 = NULL, *irows = NULL, *icols = NULL, *jqs = NULL,
      *ipointer = NULL, *nx = NULL, *ny = NULL, *nz = NULL, symmetryflag = 0,
      inputformat = 0, mei[4], ido, ldz, iparam[11], mxiter, info, ncv, jrow,
      lworkl, nev, ipntr[14], m, *select = NULL, rvec = 1, i, k, kflag, idesvar,
      node, inorm = 0, irand = 1, iinc = 1, noddiam = -1, ngraph, iobject,
      icoordinate, nrhs = 1, ithermal = 0, ishape = 0, inode, *irowd = NULL,
      *icold = NULL, *jqd = NULL, nzsc, *irowc = NULL, *icolc = NULL,
      *jqc = NULL, nevold = 0, imodes = 0, niter = 0, ifeasd = 0;

  double *xo = NULL, *yo = NULL, *zo = NULL, *x = NULL, *y = NULL, *z = NULL,
         *au = NULL, *ad = NULL, *adb = NULL, *aub = NULL, *resid = NULL,
         *workd = NULL, *workl = NULL, tol, *temp_array = NULL, *randval = NULL,
         fmin, fmax, pi, trace = 0.0, dsum = 0., *d = NULL, *zz = NULL,
         *acvector = NULL, *randfield = NULL, abserr, actreliability,
         *stn = NULL, reliability, corrlen, *acscalar = NULL, *add = NULL,
         *aud = NULL, sigma = 0, *rhs = NULL, *vector = NULL, *adbd = NULL,
  ...
  printf(
      "\n Sorting the design variables for the computation of the covariance "
      "matrix\n\n");
  ...
  mastructrand(icols, jqs, &mast1, &irows, ipointer, &nzss, ndesi, &corrlen, xo,
               yo, zo, x, y, z, nx, ny, nz);
  ...
  if (irobustdesign[2] == 1) {
    ...
    mastructcmatrix(icolc, jqc, &mast1, &irowc, ipointer, &nzsc, ndesibou,
                    nodedesibou, nodedesiinvbou, jqs, irows, icols, ndesi,
                    nodedesi);
    ...
    FORTRAN(cmatrix, (ad, au, jqs, irows, icols, ndesi, nodedesi, auc, jqc,
                      irowc, nodedesibou));
    ...
    FORTRAN(dmatrix, (ad, au, jqs, irows, icols, ndesi, nodedesi, add, aud, jqd,
                      irowd, ndesibou, nodedesibou));
    ...
    if (*isolver == 0) {
#ifdef SPOOLES
      spooles_factor(add, aud, adbd, aubd, &sigma, icold, irowd, ndesibou,
                     &nzsd, &symmetryflag, &inputformat, &nzsd);
#else
      printf(
          " *ERROR in randomfieldmain: the SPOOLES library is not linked\n\n");
      FORTRAN(stop, ());
#endif
    } else if (*isolver == 4) {
#ifdef SGI
      token = 1;
      sgi_factor(add, aud, adbd, aubd, &sigma, icold, irowd, ndesibou, &nzsd,
                 token);
#else
...
#ifdef PARDISO
      pardiso_factor(add, aud, adbd, aubd, &sigma, icold, irowd, ndesibou,
                     &nzsd, &symmetryflag, &inputformat, jqd, &nzsd);
#else
      printf(
          " *ERROR in randomfieldmain: the PARDISO library is not linked\n\n");
      FORTRAN(stop, ());
#endif
    } else if (*isolver == 8) {
#ifdef PASTIX
      pastix_factor_main(add, aud, adbd, aubd, &sigma, icold, irowd, ndesibou,
                         &nzsd, &symmetryflag, &inputformat, jqd, &nzsd);
#else
      printf(
          " *ERROR in randomfieldmain: the PASTIX library is not linked\n\n");
      FORTRAN(stop, ());
#endif
    }
    ...
    for (idesvar = 1; idesvar <= *ndesi; idesvar++) {
      ...
      FORTRAN(condrandomfield, (ad, au, jqs, irows, ndesi, rhs, vector,
                                &idesvar, jqc, auc, irowc));
    }
...
#else
    printf(" *ERROR in randomfieldmain: the MATRIXSTORAGE library is not
linked\n\n"); FORTRAN(stop,()); #endif
}*/
  else if (*isolver == 7) {
#ifdef PARDISO
  ...
  while (actreliability <= reliability) {
    ...
    FORTRAN(dsaupd, (&ido, bmat, ndesi, which, &nev, &tol, resid, &ncv, zz,
    ...
    while ((ido == -1) || (ido == 1) || (ido == 2)) {
...
#ifdef PARDISO
            pardiso_solve(temp_array, ndesi, &symmetryflag, &inputformat,
                          &nrhs);
#endif
          } else if (*isolver == 8) {
#ifdef PASTIX
      ...
      if (ido == 2) {
        opmain(ndesi, &workd[ipntr[0] - 1], &workd[ipntr[1] - 1], adb, aub, jqs,
               irows);
      }
    ...
    FORTRAN(dseupd, (&rvec, howmny, select, d, zz, &ldz, &trace, bmat, ndesi,
                     which, &nev, &tol, resid, &ncv, zz, &ldz, iparam, ipntr,
  ...
  irand = 3;
  frd_sen(co, nk, stn, inum, nmethod, kode, filab, &time, nstate_, istep, &iinc,
          &k, &noddiam, description, mi, &ngraph, ne, cs, set, nset, istartset,
          iendset, ialset, jobnamec, output, acscalar, &iobject, objectset,
          ntrans, inotr, trab, &idesvar, orname, &icoordinate, &inorm, &irand,
          &ishape, &ifeasd);
  irand = 4;
  frd_sen(co, nk, stn, inum, nmethod, kode, filab, &time, nstate_, istep, &iinc,
          &k, &noddiam, description, mi, &ngraph, ne, cs, set, nset, istartset,
          iendset, ialset, jobnamec, output, acvector, &iobject, objectset,
          ntrans, inotr, trab, &idesvar, orname, &icoordinate, &inorm, &irand,
          &ishape, &ifeasd);
  irand = 1;
  ...
  for (k = nev; k >= 1; k--) {
    ...
    for (idesvar = 0; idesvar < *ndesi; idesvar++) {
      ...
      for (i = 0; i < 3; i++) {
        acvector[3 * node + i] =
            zz[(k - 1) * *ndesi + idesvar] * extnor[3 * node + i];
      }
    }

    frd_sen(co, nk, stn, inum, nmethod, kode, filab, &d[k - 1], nstate_, istep,
            &iinc, &k, &noddiam, description, mi, &ngraph, ne, cs, set, nset,
            istartset, iendset, ialset, jobnamec, output, acscalar, &iobject,
            objectset, ntrans, inotr, trab, &idesvar, orname, &icoordinate,
            &inorm, &irand, &ishape, &ifeasd);
    irand = 2;
    frd_sen(co, nk, stn, inum, nmethod, kode, filab, &d[k - 1], nstate_, istep,
            &iinc, &k, &noddiam, description, mi, &ngraph, ne, cs, set, nset,
            istartset, iendset, ialset, jobnamec, output, acvector, &iobject,
            objectset, ntrans, inotr, trab, &idesvar, orname, &icoordinate,
            &inorm, &irand, &ishape, &ifeasd);
    irand = 1;
  }
  ...
  for (idesvar = 0; idesvar < *ndesi; idesvar++) {
    ...
    if (irobustdesign[1] == 1) {
      ...
      acscalar[node] = acscalar[node] /
                       (randomval[2 * (node) + 1] * randomval[2 * (node) + 1]);
    }
  }
