...
static ITG *ndesi1, *nodedesi1, *nodedesiinv1, *iregion1, *irow1, *icol1, *jq1,
    nzs1, *nodedesi1, *ndesi1, num_cpus, *nodedesipos1, *nsurfs1, *konfa1,
    *ipkonfa1, ndesifaces1, *idesiface1;

static double *au1 = NULL, *ad1 = NULL, *aub1 = NULL, *adb1 = NULL, *co1,
              *area1 = NULL;
...
void filterforwardmain(double *co, double *gradproj, ITG *nk, ITG *nodedesi,
  ...
  ITG *nx = NULL, *ny = NULL, *nz = NULL, i, j, *irow = NULL, *icol = NULL,
      ...
      *mast = NULL, *nodedesipos = NULL, *jq = NULL, *irowf = NULL,
      *icolf = NULL, *jqf = NULL, ndesifaces, *idesiface = NULL;

  double *xo = NULL, *yo = NULL, *zo = NULL, *x = NULL, *y = NULL, *z = NULL,
         filterrad = 0, *ad = NULL, *adb = NULL, *aub = NULL, *adb2 = NULL,
         *aub2 = NULL, sigma = 0, *rhs = NULL, *au = NULL, *adf = NULL,
         *auf = NULL, *temparray = NULL, *weighting = NULL, *area = NULL;
  ...
  FORTRAN(identdesifaces, (iregion, nsurfs, ipkonfa, lakonfa, konfa,
  ...
  konfa1 = konfa;
  lakonfa1 = lakonfa;
  nodedesipos1 = nodedesipos;
  idesiface1 = idesiface;
  ndesifaces1 = ndesifaces;
  ...
  if (strcmp1(&objectset[89], "E") == 0) {
    printf(
        " Calculation of forward filtered feasible direction with explicit "
        "method\n\n");
    ...
    mastructfilter(icolf, jqf, &mast, &irowf, ipointer, &nzs, ndesi, nodedesi,
                   xo, yo, zo, x, y, z, nx, ny, nz, &filterrad);
    ...
    FORTRAN(filterforward_exp,
            (adf, auf, jqf, irowf, ndesi, nodedesi, gradproj, feasdir,
             weighting, temparray, adb, aub, jq, irow));
    ...
    printf(
        " Calculation of forward filtered feasible direction with implicit "
        "method\n\n");
...
#else
      printf(
          "*ERROR in filterforwardmain: the SPOOLES library is not linked\n\n");
      FORTRAN(stop, ());
#endif
    } else if (*isolver == 4) {
#ifdef SGI
...
#else
      printf(
          "*ERROR in filterforwardmain: the TAUCS library is not linked\n\n");
      FORTRAN(stop, ());
#endif
    } else if (*isolver == 7) {
#ifdef PARDISO
...
#else
      printf(
          "*ERROR in filterforwardmain: the PARDISO library is not linked\n\n");
      FORTRAN(stop, ());
#endif
    } else if (*isolver == 8) {
#ifdef PASTIX
...
#else
      printf(
          "*ERROR in filterforwardmain: the PASTIX library is not linked\n\n");
      FORTRAN(stop, ());
#endif
    }
...
void *mafillmmmt2(ITG *i) {
  ...
  FORTRAN(mafillmm, (co1, nodedesiinv1, iregion1, &au1[indexau], &ad1[indexad],
                     &aub1[indexau], &adb1[indexad], irow1, jq1, ipkonfa1,
                     konfa1, lakonfa1, nodedesipos1, idesiface1, &nsurfa,
                     &nsurfb, &area1[indexad]));
