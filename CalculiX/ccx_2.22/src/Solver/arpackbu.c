...
void arpackbu(double *co, ITG *nk, ITG *kon, ITG *ipkon, char *lakon, ITG *ne,
  ...
  ITG *inum = NULL, k, ido, dz, iparam[11], ipntr[11], lworkl, im, nasym = 0,
      info, rvec = 1, *select = NULL, lfin, j, lint, iout, iconverged = 0,
      ielas = 1, icmd = 0, iinc = 1, *ncocon = NULL, *nshcon = NULL, nev, ncv,
      mxiter, jrow, coriolis = 0, ifreebody, symmetryflag = 0, storematrix = 0,
      inputformat = 0, ngraph = 1, mt = mi[1] + 1, mass[2] = {0, 0},
      stiffness = 1, buckling = 0, rhsi = 1, intscheme = 0, noddiam = -1,
      *ipneigh = NULL, *neigh = NULL, ne0, *integerglob = NULL, ntie, icfd = 0,
      *inomat = NULL, mortar = 0, *islavnode = NULL, *islavact = NULL,
      *nslavnode = NULL, *islavsurf = NULL, kscale = 1, *iponoel = NULL,
      *inoel = NULL, network = 0, nrhs = 1, *itiefac = NULL, mscalmethod = 0,
  ...
  double *stn = NULL, *v = NULL, *resid = NULL, *z = NULL, *workd = NULL,
         ...
         *cdn = NULL, *xstateini = NULL, *xstiff = NULL, *stiini = NULL,
         *vini = NULL, *stx = NULL, *enern = NULL, *xstaten = NULL, *eei = NULL,
         *enerini = NULL, *cocon = NULL, *shcon = NULL, *physcon = NULL,
         *qfx = NULL, *qfn = NULL, tol, *cgr = NULL, *xloadold = NULL, reltime,
         *vr = NULL, *vi = NULL, *stnr = NULL, *stni = NULL, *vmax = NULL,
         *stnmax = NULL, *cs = NULL, *springarea = NULL, *eenmax = NULL,
         ...
         *energy = NULL, *smscale = NULL, *aut = NULL;
  ...
  *nmethod = 1;
  ...
  if (*iperturb == 0) {
    results(
        ...
        ipompc, nodempc, coefmpc, labmpc, nmpc, nmethod, cam, &neq[0], veold,
    ...
    results(
        co, nk, kon, ipkon, lakon, ne, v, stn, inum, stx, elcon, nelcon, rhcon,
        nrhcon, alcon, nalcon, alzero, ielmat, ielorien, norien, orab, ntmat_,
        t0, t1old, ithermal, prestr, iprestr, filab, eme, emn, een, iperturb, f,
        fn, nactdof, &iout, qa, vold, b, nodeboun, ndirboun, xboun, nboun,
        ipompc, nodempc, coefmpc, labmpc, nmpc, nmethod, cam, &neq[0], veold,
        accold, &bet, &gam, &dtime, &time, ttime, plicon, nplicon, plkcon,
        nplkcon, xstateini, xstiff, xstate, npmat_, epn, matname, mi, &ielas,
  ...
  if (*iperturb == 0) {
    mafillsmmain(
        ...
        iperturb, sti, &nzs[0], stx, adb, aub, iexpl, plicon, nplicon, plkcon,
    ...
    mafillsmmain(
        ...
        vold, iperturb, sti, &nzs[0], stx, adb, aub, iexpl, plicon, nplicon,
  ...
  *nmethod = 3;
  ...
  frd(co, nk, kon, ipkon, lakon, ne, v, stn, inum, nmethod, kode, filab, een,
      t1, fn, &time, epn, ielmat, matname, enern, xstaten, nstate_, istep,
      &iinc, ithermal, qfn, &j, &noddiam, trab, inotr, ntrans, orab, ielorien,
      norien, description, ipneigh, neigh, mi, stx, vr, vi, stnr, stni, vmax,
      stnmax, &ngraph, veold, ener, ne, cs, set, nset, istartset, iendset,
      ialset, eenmax, fnr, fni, emn, thicke, jobnamec, output, qfx, cdn,
      &mortar, cdnr, cdni, nmat, ielprop, prop, sti);
  ...
  do {
    ...
    FORTRAN(dsaupd, (&ido, bmat, &neq[0], which, &nev, &tol, resid, &ncv, z,
                     &dz, iparam, ipntr, workd, workl, &lworkl, &info));
    ...
    while ((ido == -1) || (ido == 1) || (ido == 2)) {
        ...
        if (ido == -1) {
          if (*isolver == 0) {
...
#ifdef PARDISO
            pardiso_solve(temp_array, &neq[0], &symmetryflag, &inputformat,
                          &nrhs);
#endif
          } else if (*isolver == 8) {
#ifdef PASTIX
            ...
            if (pastix_solve(temp_array, &neq[0], &symmetryflag, &nrhs) == -1)
              printf(
                  " *WARNING in arpackbu: solving step didn't converge! "
                  "Continuing anyway\n");
#endif
#endif
          }
          ...
          if (*isolver == 0) {
...
#ifdef PARDISO
            pardiso_solve(&workd[ipntr[2] - 1], &neq[0], &symmetryflag,
                          &inputformat, &nrhs);
#endif
          } else if (*isolver == 8) {
#ifdef PASTIX
            ...
            pardiso_solve(&workd[ipntr[2] - 1], &neq[0], &symmetryflag,
                          &inputformat, &nrhs);
#else
            if (pastix_solve(&workd[ipntr[2] - 1], &neq[0], &symmetryflag,
                             &nrhs) == -1)
              printf(
                  " *WARNING in arpackbu: solving step didn't converge! "
                  "Continuing anyway\n");
      ...
      if (ido == 2) {
        opmain(&neq[0], &workd[ipntr[0] - 1], &workd[ipntr[1] - 1], ad, au, jq,
               irow);
      }
    ...
    */
    ...
    FORTRAN(dseupd, (&rvec, howmny, select, d, z, &dz, &sigma, bmat, &neq[0],
                     which, &nev, &tol, resid, &ncv, z, &dz, iparam, ipntr,
    ...
    /*  if((5.>d[0]/sigma)||(50000.<d[0]/sigma)){
        if(iconverged<-4) {
        printf("no convergence for the buckling factor; maybe no buckling
       occurs"); FORTRAN(stop,());
        }
        sigma=d[0]/500.;
  ...
  for (j = 0; j < nev; ++j) {
    ...
    if (*iperturb == 0) {
      results(co, nk, kon, ipkon, lakon, ne, v, stn, inum, stx, elcon, nelcon,
              ...
              labmpc, nmpc, nmethod, cam, &neq[0], veold, accold, &bet, &gam,
      ...
      results(co, nk, kon, ipkon, lakon, ne, v, stn, inum, stx, elcon, nelcon,
              rhcon, nrhcon, alcon, nalcon, alzero, ielmat, ielorien, norien,
              orab, ntmat_, t0, t1old, ithermal, prestr, iprestr, filab, eme,
    ...
    frd(co, nk, kon, ipkon, lakon, ne, v, stn, inum, nmethod, kode, filab, een,
        t1, fn, &d[j], epn, ielmat, matname, enern, xstaten, nstate_, istep,
        &iinc, ithermal, qfn, &j, &noddiam, trab, inotr, ntrans, orab, ielorien,
        norien, description, ipneigh, neigh, mi, stx, vr, vi, stnr, stni, vmax,
        stnmax, &ngraph, veold, ener, ne, cs, set, nset, istartset, iendset,
        ialset, eenmax, fnr, fni, emn, thicke, jobnamec, output, qfx, cdn,
