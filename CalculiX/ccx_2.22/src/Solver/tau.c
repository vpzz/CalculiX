...
char *taufactorooc[] = {"taucs.factor.LLT=true",
                        "taucs.factor.mf=true",
                        "taucs.factor.ordering=amd",
                        "taucs.ooc=true",
                        "taucs.ooc.basename=/tmp/scratch",
                        "taucs.ooc.memory=500000000.0",
                        NULL};
char *tausolve[] = {"taucs.factor=false", NULL};
char *tausolveooc[] = {"taucs.factor=false", "taucs.ooc.basename=/tmp/scratch",
...
void tau(double *ad, double **aup, double *adb, double *aub, double *sigma,
         double *b, ITG *icol, ITG **irowp, ITG *neq, ITG *nzs) {
  if (*neq == 0) return;

  tau_factor(ad, aup, adb, aub, sigma, icol, irowp, neq, nzs);
