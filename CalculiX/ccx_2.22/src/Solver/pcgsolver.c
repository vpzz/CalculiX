...
void PCG(double *A, double *x, double *b, ITG neq, ITG len, ITG *ia, ITG *iz,
         double *eps, ITG *niter, ITG precFlg, double *rho, double *r,
         double *g, double *C, double *z);
...
ITG cgsolver(double *A, double *x, double *b, ITG neq, ITG len, ITG *ia,
             ITG *iz, double *eps, ITG *niter, ITG precFlg) {
  ITG i = 0;
  double *Factor = NULL, *r = NULL, *p = NULL, *z = NULL, *C = NULL, *g = NULL,
         *rho = NULL;
...
/*--Preconditioning of the equation system
 * Ax+b=0----------------------------------	*/
/*---------------------------------------------------------------------------------
 */
...
void PreConditioning(double *A, ITG neq, ITG len, ITG *ia, ITG *iz,
                     double alpha, ITG precFlg, double *C, ITG *ier) {
  ITG i = 0, j = 0, jlo = 0, jup = 0, k = 0, klo = 0, kup = 0, l = 0, llo = 0,
      lup = 0;
  ITG id = 0, nILU = 0, m = 0;
  double factor;
  factor = 1.0 / (1.0 + alpha);
  /*..division of the off-diagonal elements of A by factor
   * (1.0+alpha)...............	*/
  C[0] = A[0];
  for (i = 1; i < neq; i++) {
    ...
    for (j = jlo; j < jup;
         j++) /*..all non-zero off-diagonal elements.........	*/
      C[j] = A[j] * factor;
  }
  nILU = neq; /*..ILU on full matrix.................	*/
              /*..partial Cholesky decomposition of scaled matrix
               * A..............................	*/
  for (i = 1; i < nILU; i++) {
    jlo = iz[i - 1] + 1; /*..first non-zero element in current row......
                          */
...
/*--Solution of the equation system  M rho =
 * r-------------------------------------	*/
/*---------------------------------------------------------------------------------
 */
/*--The solution of the equation system M rho = r, with M = C*CT, where C is the
 * --	*/
/*--partial Cholesky decomposition of matrix A, represent a preconditioning
 * step.--	*/
/*--The equation  system is solved by forward/backward
 * substitution.---------------	*/
/*---------------------------------------------------------------------------------
 */
/*--parameter: --	*/
...
void Mrhor(double *C, ITG neq, ITG *ia, ITG *iz, double *r, double *rho) {
  ITG i = 0, j = 0, jlo = 0, jup = 0;
  double s = 0.0;
  /*..solve equation sytem by forward/backward
   * substitution..........................	*/
  rho[0] = r[0];
  for (i = 1; i < neq; i++) {
    s = 0.0;
    ...
    for (j = jlo; j < jup;
         j++) /*..all non-zero off-diagonal element..........	*/
      s += C[j] * rho[ia[j]];
    rho[i] = (r[i] - s) / C[jup];
  }
  for (i = neq - 1; i > 0; i--) {
...
/*--Calculation of the inner product of two (distributed)
 * vectors------------------	*/
/*---------------------------------------------------------------------------------
 */
void InnerProduct(double *a, double *b, ITG n, double *Sum) {
...
void CG(double *A, double *x, double *b, ITG neq, ITG len, ITG *ia, ITG *iz,
  ...
  /*..initialize result, search and residual
   * vectors.................................	*/
  qam = 0.;
  iam = 0;
  for (i = 0; i < neq; i++) {
  ...
  for (k = 1; k <= (*niter); k++, ncg++) {
    ...
    /*......If working with Penalty-terms for boundary conditions you can get
     * nume-....	*/
    /*......rical troubles because RR becomes a very large value. With at least
     * two....	*/
     ...
     */
    qk = rr / pz;
    /*......approximated solution vector, residual
     * vector..............................	*/
    ram = 0.;
    for (i = 0; i < neq; i++) {
      x[i] = x[i] + qk * p[i];
