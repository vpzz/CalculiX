...
static void ssolve_creategraph(Graph **graph, ETree **frontETree, InpMtx *mtxA,
                               int size, FILE *msgFile) {
  IVL *adjIVL;
  int nedges;
  ...
  *graph = Graph_new();
  ...
  Graph_init2(*graph, 0, size, 0, nedges, size, nedges, adjIVL, NULL, NULL);
  if (DEBUG_LVL > 1) {
  ...
  *frontETree =
      orderViaBestOfNDandMS(*graph, TUNE_MAXDOMAINSIZE, TUNE_MAXZEROS,
                            TUNE_MAXSIZE, RNDSEED, DEBUG_LVL, msgFile);
  if (DEBUG_LVL > 1) {
...
static void ssolve_permuteA(IV **oldToNewIV, IV **newToOldIV, IVL **symbfacIVL,
                            ETree *frontETree, InpMtx *mtxA, FILE *msgFile,
                            int *symmetryflagi4) {
  int *oldToNew;
...
void factor(struct factorinfo *pfi, InpMtx *mtxA, int size, FILE *msgFile,
   ...
   *      front tree and get the symbolic factorization
   */
  ssolve_permuteA(&pfi->oldToNewIV, &pfi->newToOldIV, &symbfacIVL,
                  pfi->frontETree, mtxA, pfi->msgFile, symmetryflagi4);
   ...
   */
  {
    ...
    FrontMtx_init(pfi->frontmtx, pfi->frontETree, symbfacIVL, SPOOLES_REAL,
                  *symmetryflagi4, FRONTMTX_DENSE_FRONTS, SPOOLES_PIVOTING,
                  NO_LOCK, 0, NULL, pfi->mtxmanager, DEBUG_LVL, pfi->msgFile);
  }
   ...
   */
  {
    ...
    rootchv = FrontMtx_factorInpMtx(pfi->frontmtx, mtxA, MAGIC_TAU, MAGIC_DTOL,
                                    chvmanager, &error, pfi->cpus, stats,
...
#ifdef USE_MT
void factor_MT(struct factorinfo *pfi, InpMtx *mtxA, int size, FILE *msgFile,
               int *symmetryflagi4) {
  Graph *graph;
  IV *ownersIV;
  IVL *symbfacIVL;
   ...
   */
  ssolve_permuteA(&pfi->oldToNewIV, &pfi->newToOldIV, &symbfacIVL,
                  pfi->frontETree, mtxA, msgFile, symmetryflagi4);
   ...
   */
  {
    ...
    if (pfi->nthread > nfront) pfi->nthread = nfront;
   ...
   */
  {
    ...
    FrontMtx_init(pfi->frontmtx, pfi->frontETree, symbfacIVL, SPOOLES_REAL,
                  *symmetryflagi4, FRONTMTX_DENSE_FRONTS, SPOOLES_PIVOTING,
                  LOCK_IN_PROCESS, 0, NULL, pfi->mtxmanager, DEBUG_LVL,
                  pfi->msgFile);
  }
   ...
   */
  {
    ...
    rootchv = FrontMtx_MT_factorInpMtx(
        pfi->frontmtx, mtxA, MAGIC_TAU, MAGIC_DTOL, chvmanager, ownersIV, 0,
        &error, pfi->cpus, stats, DEBUG_LVL, pfi->msgFile);
    ChvManager_free(chvmanager);
    if (DEBUG_LVL > 1) {
      fprintf(msgFile, "\n\n factor matrix");
      FrontMtx_writeForHumanEye(pfi->frontmtx, pfi->msgFile);
   ...
   */
  {
    ...
    SolveMap_ddMap(
        pfi->solvemap, *symmetryflagi4, FrontMtx_upperBlockIVL(pfi->frontmtx),
        FrontMtx_lowerBlockIVL(pfi->frontmtx), pfi->nthread, ownersIV,
        FrontMtx_frontTree(pfi->frontmtx), RNDSEED, DEBUG_LVL, pfi->msgFile);
  }
...
DenseMtx *fsolve_MT(struct factorinfo *pfi, DenseMtx *mtxB) {
   ...
   */
  {
    ...
    DenseMtx_init(mtxX, SPOOLES_REAL, 0, 0, pfi->size, 1, 1, pfi->size);
    DenseMtx_zero(mtxX);
    FrontMtx_MT_solve(pfi->frontmtx, mtxX, mtxB, pfi->mtxmanager, pfi->solvemap,
                      pfi->cpus, DEBUG_LVL, pfi->msgFile);
    if (DEBUG_LVL > 1) {
      fprintf(pfi->msgFile, "\n\n solution matrix in new ordering");
      DenseMtx_writeForHumanEye(mtxX, pfi->msgFile);
      fflush(pfi->msgFile);
...
void spooles_factor(double *ad, double *au, double *adb, double *aub,
  ...
  if (symmetryflagi4 == 0) {
    printf(
        " Factoring the system of equations using the symmetric spooles "
        "solver\n");
  } else if ((symmetryflagi4 == 2) && (*inputformat != 4)) {
    printf(
        " Factoring the system of equations using the unsymmetric spooles "
        "solver\n");
  }
      ...
      if (*sigma == 0.) {
        for (col = 0; col < size; col++) {
          ...
          for (ipo = ipoint; ipo < ipoint + icol[col]; ipo++) {
            int row = irow[ipo] - 1;
            //			printf("row=%d,col=%d,value=%e\n",col,row,au[ipo]);
            InpMtx_inputRealEntry(mtxA, col, row, au[ipo]);
          }
        ...
        for (col = 0; col < size; col++) {
          ...
          for (ipo = ipoint; ipo < ipoint + icol[col]; ipo++) {
            int row = irow[ipo] - 1;
            InpMtx_inputRealEntry(mtxA, col, row, au[ipo] - *sigma * aub[ipo]);
          }
          ipoint = ipoint + icol[col];
        }
      }
      ...
      if (*sigma == 0.) {
        for (col = 0; col < size; col++) {
          ...
          for (ipo = ipoint; ipo < ipoint + icol[col]; ipo++) {
            ...
            //			printf("row=%d,col=%d,value=%e\n",col,row,au[ipo+*nzs]);
            InpMtx_inputRealEntry(mtxA, col, row, au[ipo + (int)*nzs3]);
          }
          ipoint = ipoint + icol[col];
        }
      } else {
        for (col = 0; col < size; col++) {
          ...
          for (ipo = ipoint; ipo < ipoint + icol[col]; ipo++) {
            int row = irow[ipo] - 1;
            InpMtx_inputRealEntry(mtxA, row, col, au[ipo] - *sigma * aub[ipo]);
      ...
      for (i = 0; i < *neq; i++) {
        for (j = 0; j < *neq; j++) {
          if (fabs(ad[i * (int)*nzs + j]) > 1.e-20) {
            InpMtx_inputRealEntry(mtxA, j, i, ad[i * (int)*nzs + j]);
          }
        }
      }
...
void spooles_solve(double *b, ITG *neq) {
  ...
  //	printf(" Solving the system of equations using the symmetric spooles
  // solver\n");
...
void spooles_cleanup() {
  ...
  if (pfi.solvemap) SolveMap_free(pfi.solvemap);
...
void spooles_factor_rad(double *ad, double *au, double *adb, double *aub,
                        double *sigma, ITG *icol, ITG *irow, ITG *neq, ITG *nzs,
                        ITG *symmetryflag, ITG *inputformat, ITG *iexpl) {
  int symmetryflagi4 = *symmetryflag;
  int size = *neq;
  InpMtx *mtxA;
  ...
  if (*iexpl <= 1) {
    if (symmetryflagi4 == 0) {
      ...
      printf(
          " Factoring the system of radiation equations using the unsymmetric "
          "spooles solver\n");
    }
  }
      ...
      if (*sigma == 0.) {
        for (col = 0; col < size; col++) {
          ...
          for (ipo = ipoint; ipo < ipoint + icol[col]; ipo++) {
            int row = irow[ipo] - 1;
            InpMtx_inputRealEntry(mtxA, row, col, au[ipo]);
            InpMtx_inputRealEntry(mtxA, col, row, au[ipo + (int)*nzs]);
          }
          ipoint = ipoint + icol[col];
        }
      } else {
...
void spooles_solve_rad(double *b, ITG *neq) {
  ...
  //	printf(" Solving the system of radiation equations using the unsymmetric
  // spooles solver\n");
...
void spooles_cleanup_rad() {
  ...
  if (pfj.solvemap) SolveMap_free(pfj.solvemap);
