...
void pardiso_factor(double *ad, double *au, double *adb, double *aub,
  ...
  ITG i, j, k, l, maxfct = 1, mnum = 1, phase = 12, nrhs = 1, *perm = NULL,
                  mtype, msglvl = 0, error = 0, *irowpardiso = NULL, kflag,
  ...
  if (*symmetryflag == 0) {
    ...
    if (*inputformat == 3) {
      ...
      kflag = 2;
      FORTRAN(isortiid, (irowpardiso, icolpardiso, aupardiso, &ndim, &kflag));
      ...
      for (i = 0; i < *neq; i++) {
        ...
        do {
          if (irowpardiso[k] != j) {
            n = k - kstart;
            FORTRAN(isortid,
                    (&icolpardiso[kstart], &aupardiso[kstart], &n, &kflag));
            pointers[i + 1] = k + 1;
            break;
          } else {
            if (k + 1 == ndim) {
  ...
  FORTRAN(pardiso,
          (pt, &maxfct, &mnum, &mtype, &phase, neq, aupardiso, pointers,
           icolpardiso, perm, &nrhs, iparm, &msglvl, b, x, &error));
