...
void pardiso_factor_cp(double *ad, double *au, double *adb, double *aub,
  ...
  char *env;
  /*  char env1[32]; */
  ITG i, j, k, l, maxfct = 1, mnum = 1, phase = 12, nrhs = 1, *perm = NULL,
                  mtype, msglvl = 0, error = 0, *irowpardisocp = NULL, kflag,
                  kstart, n, ifortran, lfortran, index, id, k2;
  ...
  if (*iexpl <= 1) {
    if (*symmetryflag == 0) {
      printf(
          " Factoring the system of equations using the symmetric pardiso "
          "solver\n");
    } else {
      printf(
          " Factoring the system of equations using the unsymmetric pardiso "
          "solver\n");
    }
  }
  ...
  if (*symmetryflag == 0) {
    ...
    if (*inputformat == 3) {
      ...
      kflag = 2;
      FORTRAN(isortiid,
              (irowpardisocp, icolpardisocp, aupardisocp, &ndim, &kflag));
      ...
      for (i = 0; i < *neq; i++) {
        ...
        do {
          if (irowpardisocp[k] != j) {
            n = k - kstart;
            FORTRAN(isortid,
                    (&icolpardisocp[kstart], &aupardisocp[kstart], &n, &kflag));
            pointerscp[i + 1] = k + 1;
            break;
          } else {
            if (k + 1 == ndim) {
  ...
  FORTRAN(pardiso,
          (ptcp, &maxfct, &mnum, &mtype, &phase, neq, aupardisocp, pointerscp,
           icolpardisocp, perm, &nrhs, iparmcp, &msglvl, b, x, &error));
...
void pardiso_solve_cp(double *b, ITG *neq, ITG *symmetryflag, ITG *inputformat,
                      ITG *nrhs) {
  ITG maxfct = 1, mnum = 1, phase = 33, *perm = NULL, mtype, msglvl = 0, i,
      error = 0;
  double *x = NULL;

  /* if(*symmetryflag==0){
    printf(" Solving the system of equations using the symmetric pardiso
  solver\n"); }else{ printf(" Solving the system of equations using the
  unsymmetric pardiso solver\n");
    }*/
  ...
  /* pardiso_factor_cp has been called befor, MKL_NUM_THREADS=mthread_mkl_cp is
   * set*/
...
void pardiso_main_cp(double *ad, double *au, double *adb, double *aub,
  ...
  pardiso_factor_cp(ad, au, adb, aub, sigma, icol, irow, neq, nzs, symmetryflag,
                    inputformat, jq, nzs3, &iexpl);
