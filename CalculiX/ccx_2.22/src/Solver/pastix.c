...
// This is set to one, when to many iterations with mixed precision did not
// converge
...
pastix_data_object pastix_mode_basic = {0,
                                        0,
                                        NULL,
                                        NULL,
                                        0,
...
pastix_data_object pastix_mode_as = {
    ...
    NULL, NULL, NULL, NULL,      1,         0, 0,    0,    1,    0,  0,
    0,    NULL, NULL, piparm_as, pdparm_as, 0, 1,    NULL, NULL};

pastix_data_object pastix_mode_cp = {
    0,    0,    NULL, NULL,      0,         0, NULL, NULL, NULL, -1, 0,
    NULL, NULL, NULL, NULL,      1,         0, 0,    0,    1,    0,  0,
    0,    NULL, NULL, piparm_cp, pdparm_cp, 0, 1,    NULL, NULL};

// Initializes and configurates PaStiX environment. Also forwards the sparse
// matrix pointers
void pastix_init(double *ad, double *au, double *adb, double *aub,
  ...
  char usage_call = MULTI_SOLVE if (usage == usage_call) {
  ...
  // perform reordering, analysis and symbolic factorization if it's more than 1
  // equation
  if (spm->n > 1) {
...
void pastix_csc_conversion(double *ad, double *au, double *adb, double *aub,
  ...
  if (*neq != neqPrev || *inputformat != inputformatPrev) forceRedo = 1;
...
#pragma omp parallel for reduction(+ : nzsTotal)
    ...
    // If the number of entries in the merged matrix is the same as in the last
    // iteration, we can reuse
    if (nzsTotal == nzsPrev) {
      printf("Reusing csc.!\n");
    } else {
      redo = 1;
...
#pragma omp parallel for shared(auPtr)
    for (i = 0; i < *neq; i++) {
      ...
      while (kCur < jq[i + 1] - base && kPrev < jqPrev[i + 1] - base) {
        if (irowPrev[kPrev] == irow[kCur]) {
          ...
          if (*symmetryflag && *inputformat != 3)
            auPtr[kTotal + nzsTotal] =
                au[kCur + *nzs3] -
                (*sigma == 0 ? 0.0 : (*sigma) * aub[kCur + *nzs3]);
          ...
          {
            auPtr[kTotal] =
                au[kCur] - (*sigma == 0 ? 0.0 : (*sigma) * aub[kCur]);
    ...
    // This is executed in either the first iteration, or when the number of
    // equations changed
  ...
  if (*inputformat == 1 || *symmetryflag == 0) {
    ...
    if (redo) {
      // We allocate 10% more space for the values than required so that we have
      // to perform the expensive cudaMallocHost only once, even when the size
      // of the matrix increases slightly
      if ((nzsTotal * 2 + *neq) > pastix_nnzBound) {
        // perform the call with PaStiX because pinned memory allocation via
        // CUDA is performed if gpu is activated
        if (!firstIter && aupastix == spm->values) spm->values = NULL;
        pastixAllocMemory((void **)&aupastix,
                          sizeof(double) * 1.1 * (nzsTotal * 2 + *neq), gpu);
    ...
    // irowPrediction stores the offset to the first entry in it's column of
    // each entry irowacc stores the number of elements in each row
    if (redo) {
...
#pragma omp parallel for private(j) shared(aupastix)
    for (i = 0; i < *neq; i++) {
      ...
      aupastix[k_pastix - 1] =
          ad[i] -
          (merged != 0 ? 0.0 : (*sigma == 0.0 ? 0.0 : (*sigma) * adb[i]));
...
#pragma omp parallel for private(j) shared(aupastix)
    for (i = 0; i < *neq; i++) {
      ITG k = jqTotal[i] - 1;
      for (j = 0; j < icol[i]; j++) {
        aupastix[irowPrediction[k] + icolpastix[irow[k] - 1] - 1] =
            au[k +
               (*symmetryflag == 0 ? 0 : (*nzs == *nzs3 ? nzsTotal : *nzs3))] -
            (merged != 0
                 ? 0
                 : (*sigma == 0.0
                        ? 0.0
                        : (*sigma * aub[k + (*symmetryflag == 0
                                                 ? 0
                                                 : (*nzs == *nzs3 ? nzsTotal
                                                                  : *nzs3))])));
        k++;
...
#pragma omp parallel for private(j, z) firstprivate(temp, found)
      for (i = 0; i < *neq; i++) {
        ...
        for (j = jqTotal[i] - 1; j < jqTotal[i + 1] - 1; j++) {
          temp = irow[j];
          // loop through the symmetric column counter part to check for
          // symmetry
...
// if no entry was found add a dummy to the array and increase the counter for
// missing entries
#pragma omp critical
          if (found == 0) {
      ...
      // allocate memory for the PaStiX arrays and free the old ones if
      // necessary
      if ((nzsTotal + *neq) > pastix_nnzBound) {
        if (!firstIter && aupastix == spm->values) spm->values = NULL;
        pastixAllocMemory((void **)&aupastix,
                          sizeof(double) * 1.1 * (nzsTotal + *neq), gpu);
    ...
    for (i = 0; i < *neq; i++) {
      ...
      if (redo) {
        ...
        memcpy(irowpastix + i + offsetSource + offsetPrev[i],
               irow + offsetSource, sizeof(ITG) * entriesPerColumn);
      ...
      if (*sigma != 0 && merged == 0) {
        for (j = 0; j < entriesPerColumn; j++) {
          aupastix[i + offsetSource + j + offsetPrev[i]] -=
              (*sigma) * aub[offsetSource + j];
        }
      }

      // add the diagonal entries to aupastix
      aupastix[i + (jqTotal[i + 1] - jqTotal[0]) + offsetPrev[i]] =
          ad[i] - (merged != 0 ? 0.0 : (*sigma == 0 ? 0.0 : (*sigma) * adb[i]));

      // add the found entries for making the matrix structural symmetric and
      // increase the resulting offset in arrays
      if (redo) {
        offsetPrev[i + 1] = offsetPrev[i];
        for (j = 0; j < countnew; j++) {
          if (col_newEntries[j] - 1 == i) {
            irowpastix[i + (jqTotal[i + 1] - jqTotal[0]) + 1 +
                       offsetPrev[i + 1]] = row_newEntries[j];
            offsetPrev[i + 1]++;
          }
        }
...
void pastix_factor_main_generic(double *ad, double *au, double *adb,
  ...
  // Perform individual invocations always in double precision. If previous
  // iterations were in single precision, do not reuse.
  forceRedo = 1;
  globDoublePrecision = 1;

  // invoke PaStiX
  pastix_csc_conversion(ad, au, adb, aub, sigma, icol, irow, neq, nzs,
                        symmetryflag, inputformat, jq, nzs3);
  pastix_cleanup(neq, symmetryflag);
  pastix_init(ad, au, adb, aub, sigma, icol, irow, neq, nzs, symmetryflag,
              inputformat, jq, nzs3);
  gpu = 0;
  piparm[IPARM_GPU_NBR] = 0;
  pastix_data->piparm[IPARM_GPU_NBR] = 0;
  pastix_factor(ad, au, adb, aub, sigma, icol, irow, neq, nzs, symmetryflag,
                inputformat, jq, nzs3);
}
...
ITG pastix_solve_generic(double *x, ITG *neq, ITG *symmetryflag, ITG *nrhs) {
  ...
  // If we are in mixed precision mode, cast double x to float x and call solve.
  // Afterwards upcast the solution.
...
#pragma omp parallel for
  ...
  char usage_call = SINGLE_SOLVE if (usage == usage_call || rc != 0) {
    // invoke iterative refinement in double precision
    rc = pastix_task_refine(pastix_data, spm->n, *nrhs, (void *)b, spm->n,
                            (void *)x, spm->n);
...
// Invokes pastixFinalize and spmExit which frees everything but the dense LU
// array and parsec pointer
...
void pastix_main_generic(double *ad, double *au, double *adb, double *aub,
  ...
  const char *pastix_gpu = getenv("PASTIX_GPU");
  if (pastix_gpu) gpu = (*pastix_gpu == '1') ? 1 : 0;
  ...
  // use double precision for inputformat 3 like mortar (better perfromance and
  // convergence)
  ...
  double pastixTime, stepCscConversion, stepInit, stepFactorize, stepSolve,
      stepCleanUp, totalCCXTime, CCXwithoutPastix;
...
#pragma omp parallel for
  ...
  if (rc == -1) {
    ...
    if (globDoublePrecision == 1) {
      ...
      pastix_main_generic(ad, au, adb, aub, sigma, b, icol, irow, neq, nzs,
                          symmetryflag, inputformat, jq, nzs3, nrhs);
    }
    ...
    // if we do not converge with mixed precision for the third time,
    // permanently switch to double precision
    if (mixedFailed <= 2) {
      stickToDouble = 0;
      forceRedo = 1;
    }
  ...
  totalCCXTime = (totalCCXTime + (totalCalculixTimeEnd.tv_nsec -
  ...
  stepCscConversion =
      (stepCscConversionEnd.tv_sec - stepCscConversionStart.tv_sec) * 1e9;
  stepCscConversion = (stepCscConversion + (stepCscConversionEnd.tv_nsec -
                                            stepCscConversionStart.tv_nsec)) *
                      1e-9;
  ...
  stepFactorize = (stepFactorize +
                   (stepFactorizeEnd.tv_nsec - stepFactorizeStart.tv_nsec)) *
                  1e-9;

  stepSolve = (stepSolveEnd.tv_sec - stepSolveStart.tv_sec) * 1e9;
  stepSolve =
      (stepSolve + (stepSolveEnd.tv_nsec - stepSolveStart.tv_nsec)) * 1e-9;

  stepCleanUp = (stepCleanUpEnd.tv_sec - stepCleanUpStart.tv_sec) * 1e9;
  stepCleanUp =
      (stepCleanUp + (stepCleanUpEnd.tv_nsec - stepCleanUpStart.tv_nsec)) *
      1e-9;
...
void pastix_factor_main(double *ad, double *au, double *adb, double *aub,
  ...
  mode = BASIC;
  usage = MULTI_SOLVE;
  pastix_factor_main_generic(ad, au, adb, aub, sigma, icol, irow, neq, nzs,
                             symmetryflag, inputformat, jq, nzs3);
