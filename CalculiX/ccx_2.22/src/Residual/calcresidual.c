...
void calcresidual(ITG *nmethod, ITG *neq, double *b, double *fext, double *f,
                  ...
                  double *adb, double *aub, ITG *jq, ITG *irow, ITG *nzl,
  ...
  /* residual for implicit dynamics */
    ...
    for (k = 0; k < neq[0]; ++k) {
      b[k] =
          scal1 * (fext[k] - f[k]) - alpha[0] * (fextini[k] - fini[k]) - b[k];
    }
  ...
  /* residual for explicit dynamics */
    ...
    scal1 = 1. + alpha[0];
    res2parll(b, &scal1, fext, f, alpha, fextini, fini, adb, aux2, &neq[0],
              num_cpus);
    for (k = neq[0]; k < neq[1]; ++k) {
    ...
    if (*idamping == 1) {
      ...
      res4parll(cv, alpham, adb, aux2, b, &scal1, alpha, cvini, &neq[0],
                num_cpus);
    }
  }
