...
void getuncrackedresults(char *masterfile, ITG **integerglobp,
  ...
  ITG *kontet = NULL, *ifatet = NULL, *inodfa = NULL, *ipofa = NULL, type, n1,
      ...
      *integerglob = NULL, *ielemnr = NULL, istep_global, ioffset;
  ...
  if ((anz[0].n == 0) || (anz[0].e == 0)) {
    printf(
        " *ERROR in getuncrackedresults: there are either no nodes or\n no "
    ...
    if (type == 1) {
      ...
      for (j = 0; j < 6; j++) {
        ...
        FORTRAN(createtet, (kontet, ifatet, &netet, inodfa, &ifreefa, planfa,
                            ipofa, nodes, cotet, &iparent[netet]));
      }
    } else if (type == 2) {
      /* C3D6 */
    ...
    if (loadcase > -1) {
      *nstep = max(*nstep, istep_global);
      if (!read_mode && readfrdblock(loadcase, anz, node, lcase) == -1) {
        printf(
            " *ERROR in getuncrackedresults: Could not read data for "
            "Dataset:%" ITGFORMAT "\n",
            i + 1);
        FORTRAN(stop, ());
      }
  ...
  for (i = 0; i < anz[0].l; i++) {
    ...
    if (loadcase > -1) {
      *nstep = max(*nstep, istep_global);
      ...
      for (j = 0; j < anz[0].n; j++) {
        nodenr = node[j].nr;
        field[ioffset + 13 * (nodenr - 1) + 10] =
            lcase[loadcase].dat[0][nodenr];
        field[ioffset + 13 * (nodenr - 1) + 11] =
            lcase[loadcase].dat[1][nodenr];
        field[ioffset + 13 * (nodenr - 1) + 12] =
            lcase[loadcase].dat[2][nodenr];
      }
    }
  }
  ...
  NNEW(doubleglob, double,
       *nstep * 13 * nktet + 3 * nktet + 4 * nfaces + 6 * netet);
