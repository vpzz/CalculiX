...
void mulmatvec_asymmain(double *au, ITG *jq, ITG *irow, ITG *ncol, double *x,
  ...
  if (*itranspose == 0) {
    ...
    for (i = 0; i < num_cpus; i++) {
      ithread[i] = i;
      pthread_create(&tid[i], NULL, (void *)mulmatvec_asymmt1,
                     (void *)&ithread[i]);
    }
    ...
    for (i = 0; i < num_cpus; i++) {
      ithread[i] = i;
      pthread_create(&tid[i], NULL, (void *)mulmatvec_asymct1,
                     (void *)&ithread[i]);
    }
    for (i = 0; i < num_cpus; i++) pthread_join(tid[i], NULL);
    ...
    for (i = 0; i < num_cpus; i++) {
      ithread[i] = i;
      pthread_create(&tid[i], NULL, (void *)mulmatvec_asymmt2,
                     (void *)&ithread[i]);
    }
    for (i = 0; i < num_cpus; i++) pthread_join(tid[i], NULL);
...
void *mulmatvec_asymmt1(ITG *i) {
  ...
  FORTRAN(mulmatvec_asym,
          (au1, jq1, irow1, x1, &yy[indexf], itranspose1, &ncola, &ncolb));
...
void *mulmatvec_asymmt2(ITG *i) {
  ...
  FORTRAN(mulmatvec_asym,
          (au1, jq1, irow1, x1, yy1, itranspose1, &ncola, &ncolb));
