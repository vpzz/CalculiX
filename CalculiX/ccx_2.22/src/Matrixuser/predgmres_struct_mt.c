...
void predgmres_struct_mt(double *ad, double **aup, double *adb, double *aub,
  ...
  ITG i, j, k, l, *irowpardiso = NULL, kflag, kstart, n, k2, ndim, num_cpus,
                  isym, itol, itmax, iter, ierr, iunit, lrgw, *igwk = NULL,
                  ligw, *iwork = NULL, *icolpardiso = NULL, *ipointers = NULL,
                  *nestart = NULL, *ja = NULL, nz_num;

  double *sb = NULL, *sx = NULL, *rgwk = NULL, tol, err, *rwork = NULL,
         *x = NULL, *aupardiso = NULL, *au = NULL, dgmrestol = 1.e-12;
  ...
  printf(
      " Factoring and solving the system of equations using the gmres "
      "solver\n");
  ...
  dgmresmain(neq, b, x, &nz_num, icolpardiso, ja, aupardiso, &isym, &itol, &tol,
             &itmax, &iter, &err, &ierr, &iunit, sb, sx, rgwk, &lrgw, igwk,
             &ligw, rwork, iwork, nestart, &num_cpus, &dgmrestol);

  if (ierr > 0) {
    printf(
        " *WARNING in predgmres_struct_mt: error message from dgmresmain "
        "=%d\n\n",
        ierr);
  }
