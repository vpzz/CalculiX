  ...
  for (i = 0; i < MAX_LINE_LENGTH; i++) {
    ...
    if ((rec_str[i] == breakchar) && (!skip_breakchar)) {
      ...
      if (strlen(dat[nextarg])) {
        nextarg++;
        letter = 0;
        if ((dat = (char **)realloc((char **)dat,
                                    (nextarg + 1) * sizeof(char *))) == NULL)
          printf(" ERROR: realloc failed\n");
        if ((dat[nextarg] = (char *)malloc(MAX_LINE_LENGTH * sizeof(char))) ==
            NULL)
          printf(" ERROR: malloc failed\n");
        for (j = 0; j < MAX_LINE_LENGTH; j++) dat[nextarg][j] = '\0';
      }
