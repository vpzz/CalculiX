...
void readinput(char *jobnamec, char **inpcp, ITG *nline, ITG *nset, ITG *ipoinp,
               ITG **inpp, ITG **ipoinpcp, ITG *ithermal, ITG *nuel,
               ITG *inp_size) {
  ...
  ITG i, j, k, n,
      in = 0, nlinemax = 100000, irestartread, irestartstep, icntrl, nload,
      nforc, nboun, nk, ne, nmpc, nalset, nmat, ntmat, npmat, norien, nam,
      nprint, mi[3], ntrans, ncs, namtot, ncmat, memmpc, ne1d, ne2d, nflow,
      *meminset = NULL, *rmeminset = NULL, *inp = NULL, ntie, nener, nstate,
      nentries = 19, ifreeinp, ikey, lincludefn, nslavs, nbody,
      ncharmax = 1000000, *ipoinpc = NULL, ichangefriction = 0, nkon, ifile,
      mcs, initialtemperature = 0, nprop, mortar, ifacecount, nintpoint,
      infree[4], iheading = 0, ichangesurfacebehavior = 0, nef, mpcend,
      nheading_, network, nfc, ndc, iprestr;
  ...
  *nline = 0;
  ...
  do {
    ...
    if (strcmp1(&buff[0], "*INCLUDE") == 0) {
      ...
      if (in > 9) {
        printf(
            " *ERROR in readinput: include statements can \n not be cascaded "
            "over more than 9 levels\n");
        exit(201);
      }
    ...
    if (strcmp1(&buff[0], "*AMPLITUDE") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "AMPLITUDE", nline, &ikey));
      ...
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "MATERIAL", nline, &ikey));
    } else if (strcmp1(&buff[0], "*CONTACTDAMPING") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "INTERACTION", nline, &ikey));
    } else if (strcmp1(&buff[0], "*CONTACTPAIR") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "CONTACTPAIR", nline, &ikey));
    } else if (strcmp1(&buff[0], "*COUPLING") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "COUPLING", nline, &ikey));
      ...
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "DISTRIBUTION", nline, &ikey));
      ...
      (*nset)++;
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "ELEMENT", nline, &ikey));
    } else if (strcmp1(&buff[0], "*ELSET") == 0) {
      (*nset)++;
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "ELSET", nline, &ikey));
    } else if (strcmp1(&buff[0], "*EXPANSION") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "MATERIAL", nline, &ikey));
    } else if (strcmp1(&buff[0], "*FLUIDCONSTANTS") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "MATERIAL", nline, &ikey));
    } else if ((strcmp1(&buff[0], "*FRICTION") == 0) &&
               (ichangefriction == 0)) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "INTERACTION", nline, &ikey));
      ...
      FORTRAN(keystart,
              (&ifreeinp, ipoinp, inp, "INITIALCONDITIONS", nline, &ikey));
      ...
      FORTRAN(keystart,
              (&ifreeinp, ipoinp, inp, "MATRIXASSEMBLE", nline, &ikey));
      (*nuel)++;
      ...
      (*nset)++;
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "NODE", nline, &ikey));
    } else if (strcmp1(&buff[0], "*NSET") == 0) {
      (*nset)++;
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "NSET", nline, &ikey));
    } else if (strcmp1(&buff[0], "*ORIENTATION") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "ORIENTATION", nline, &ikey));
      ...
      if (irestartread == 1) {
        icntrl = 0;
        FORTRAN(restartshort,
                (nset, &nload, &nbody, &nforc, &nboun, &nk, &ne, &nmpc, &nalset,
                 &nmat, &ntmat, &npmat, &norien, &nam, &nprint, mi, &ntrans,
                 &ncs, &namtot, &ncmat, &memmpc, &ne1d, &ne2d, &nflow, set,
                 meminset, rmeminset, jobnamec, &irestartstep, &icntrl,
                 ithermal, &nener, &nstate, &ntie, &nslavs, &nkon, &mcs, &nprop,
                 &mortar, &ifacecount, &nintpoint, infree, &nef, &mpcend,
                 &nheading_, &network, &nfc, &ndc, &iprestr));
        FORTRAN(keystart,
                (&ifreeinp, ipoinp, inp, "RESTART,READ", nline, &ikey));
      } else {
        FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "REST", nline, &ikey));
      }
      ...
      if (ichangesurfacebehavior == 0) {
        FORTRAN(keystart,
                (&ifreeinp, ipoinp, inp, "INTERACTION", nline, &ikey));
      } else {
        FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "REST", nline, &ikey));
      }
    } else if (strcmp1(&buff[0], "*SURFACE") == 0) {
      (*nset)++;
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "SURFACE", nline, &ikey));
    } else if (strcmp1(&buff[0], "*TIE") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "TIE", nline, &ikey));
    } else if (strcmp1(&buff[0], "*TRANSFORM") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "TRANSFORM", nline, &ikey));
    } else if (strcmp1(&buff[0], "*USERELEMENT") == 0) {
      FORTRAN(keystart, (&ifreeinp, ipoinp, inp, "USERELEMENT", nline, &ikey));
      (*nuel)++;
