...
void checkconvergence(
    ...
    double *energyref, double *emax, double *r_abs, double *enetoll,
    double *energyini, double *allwkini, double *temax, double *sizemaxinc,
    ITG *ne0, ITG *neini, double *dampwk, double *dampwkini,
    double *energystartstep) {
  ITG i0, ir, ip, ic, il, ig, ia, iest,
      iest1 = 0, iest2 = 0, iconvergence, idivergence, ngraph = 1, k,
      *ipneigh = NULL, *neigh = NULL, *inum = NULL, id, istart, iend, inew, i,
      j, mt = mi[1] + 1, iexceed, iforceincsize = 0, kscalemax, itf2f;

  double df, dc, db, dd, ran, can, rap, ea, cae, ral, da,
      *vr = NULL, *vi = NULL, *stnr = NULL, *stni = NULL, *vmax = NULL,
      *stnmax = NULL, *cs = NULL, c1[2], c2[2], reftime, *fn = NULL,
      *eenmax = NULL, *fnr = NULL, *fni = NULL, *qfx = NULL, *cdn = NULL,
  ...
  *ialeatoric = 0;
    ...
    if (((*iit > 1) && (ram[0] <= c1[0] * qam[0]) &&
         ...
         ((cam[0] <= c2[0] * uam[0]) ||
          (((ram[0] * cam[0] < c2[0] * uam[0] * ram2[0]) ||
           ...
           (*ntg == 0)) ||
          (cam[0] < 1.e-8))) &&
        ((ram[1] <= c1[1] * qam[1]) && (cam[2] < *deltmx) &&
         ((cam[1] <= c2[1] * uam[1]) ||
          (((ram[1] * cam[1] < c2[1] * uam[1] * ram2[1]) ||
            (ram[1] <= ral * qam[1]) || (qa[1] <= ea * qam[1])) &&
           (*ntg == 0)) ||
          (cam[1] < 1.e-8))))
      iconvergence = 1;
  }
  ...
  if (iconvergence == 1) {
    if (*kscale > 1) {
      *kscale = 1;
      iconvergence = 0;
      printf(
          "\n restoring the elastic contact stifnesses to their original "
          "values \n\n");
    }
  }
  ...
  if ((*nmethod == 4) && (*ithermal < 2) && (*uncoupled == 0) &&
      (iconvergence == 1) && (*ne == *ne0) && (*neini == *ne0) &&
      (*idrct == 0)) {
    ...
    *r_abs = energy[0] + energy[1] + energy[2] + energy[3] - *energyref -
             *allwk - *dampwk;

    // Absolute tolerance check (when the error is really small --> beginning of
    // simulation)
  ...
  if ((*nmethod == 4) && (*ithermal < 2) && (*uncoupled == 0) &&
    ...
    *emax = max(*emax, fabs(*allwk));
    ...
    FORTRAN(checkimpacts, (ne, neini, temax, sizemaxinc, energyref, tmin, tmax,
                           tper, &idivergence, &iforceincsize, istab, dtheta,
                           r_abs, energy, energyini, allwk, allwkini, dampwk,
                           dampwkini, emax, mortar, &maxdecay, enetoll));
    ...
    if ((*ne == *ne0) && (*neini > *ne0) && (idivergence == 0)) {
      *r_abs = fabs(energy[0] + energy[1] + energy[2] + energy[3] - *energyref -
                    *allwk - *dampwk);
    ...
    /*
      ...
      Take care of the general (increasing) trend of the residual to avoid code
      stucks.
    */

    if ((iconvergence == 1) && (*ne <= *neini)) {
      tmp = energy[0] + energy[1] + energy[2] + energy[3] - *energyref -
            *allwk - *dampwk;
      if (tmp > *r_abs) {
        *r_abs = tmp;
  ...
  if ((iconvergence == 1) && (idivergence == 0)) {
    ...
    *theta = *theta + *dtheta;
      ...
      if (*mortar == 0) {
        ...
        *dthetaref = *dtheta;
        ...
        if (*dtheta < *tmin) {
          ...
          FORTRAN(storeresidual,
                  (nactdof, b, fn, filab, ithermal, nk, sti, stn, ipkon, inum,
                   kon, lakon, ne, mi, orab, ielorien, co, itg, ntg, vold,
                   ielmat, thicke, ielprop, prop));
          ++*kode;

          (*ttime) += (*time);
          frd(co, nk, kon, ipkon, lakon, ne, vold, stn, inum, nmethod, kode,
              filab, een, t1act, fn, ttime, epn, ielmat, matname, enern,
              xstaten, nstate_, istep, iinc, ithermal, qfn, mode, noddiam, trab,
              inotr, ntrans, orab, ielorien, norien, description, ipneigh,
              neigh, mi, sti, vr, vi, stnr, stni, vmax, stnmax, &ngraph, veold,
              ener, ne, cs, set, nset, istartset, iendset, ialset, eenmax, fnr,
              fni, emn, thicke, jobnamec, output, qfx, cdn, mortar, cdnr, cdni,
              nmat, ielprop, prop, sti);
    ...
    if ((*dtheta > *sizemaxinc) && (*idrct == 0)) {
      ...
      *dthetaref = *dtheta;
      printf(
          " the increment size exceeds the maximum allowed and is decreased to "
          "%e\n\n",
          *dtheta * *tper);
    }
    ...
    if ((*itpamp > 0) && (*idrct == 0)) {
      ...
      if ((*inext == inew) && (inew <= iend)) {
        if (amta[2 * inew - 2] - reftime < 1.e-6 * *tper) {
          inew++;
        }
      }
      ...
      if (*inext < inew) {
        ...
        *itp = 1;
        printf(
            " the increment size exceeds a time point and is decreased to "
            "%e\n\n",
            *dtheta * *tper);
      } else {
        *itp = 0;
    ...
    if (((*iit > ic) && (*mortar == 0)) || ((*mortar > 1) && (*iit > 200))) {
      ...
      for (k = 0; k < *nk; k++) inum[k] = 1;
      FORTRAN(storeresidual,
              (nactdof, b, fn, filab, ithermal, nk, sti, stn, ipkon, inum, kon,
               lakon, ne, mi, orab, ielorien, co, itg, ntg, vold, ielmat,
               thicke, ielprop, prop));
      ++*kode;

      (*ttime) += (*time);
      frd(co, nk, kon, ipkon, lakon, ne, vold, stn, inum, nmethod, kode, filab,
          een, t1act, fn, ttime, epn, ielmat, matname, enern, xstaten, nstate_,
          istep, iinc, ithermal, qfn, mode, noddiam, trab, inotr, ntrans, orab,
    ...
    if ((*iit >= i0) || (fabs(ram[0]) > 1.e20) || (fabs(cam[0]) > 1.e20) ||
      ...
      if ((*ithermal != 2) && (*mortar == 1)) {
        ...
        if (((ITG)ram[5] * (ITG)ram1[5] < 0) &&
            ((ITG)ram1[5] * (ITG)ram2[5] < 0)) {
      ...
      if (idivergence > 0) {
        if (*idrct == 1) {
          ...
          if (qa[2] > 0.) {
            *dtheta = *dtheta * qa[2];
            printf(
                "increment size decrease requested by a material user routine "
                "(through pnewdt)\n\n");
          } else {
            if (idivergence == 1) {
        ...
        if (*idrct != 1) {
          ...
          *dthetaref = *dtheta;
          if ((*mortar == 1) && (*iit == itf2f)) {
            printf(
                "maximum number of iterations for face-to-face contact "
                "reached\n");
            printf(" the increment size is decreased to %e\n", *dtheta * *tper);
            printf(" the increment is reattempted\n\n");
          } else {
            printf(
                " too slow convergence; the increment size is decreased to "
                "%e\n",
                *dtheta * *tper);
            printf(" the increment is reattempted\n\n");
          }
