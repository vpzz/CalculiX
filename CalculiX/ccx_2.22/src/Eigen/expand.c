...
void expand(double *co, ITG *nk, ITG *kon, ITG *ipkon, char *lakon, ITG *ne,
            ...
            double *fmpc, ITG *nev, double **zp, ITG *iamboun, double *xbounold,
            ...
            double *prop, char *orname, ITG *itiefac, double *t0g,
            double *t1g) {
  /* calls the Arnoldi Package (ARPACK) for cyclic symmetry calculations */

  char *filabt, lakonl[2] = " \0";

  ITG *inum = NULL, k, idir, j, iout = 0, index, inode, id, i, idof, im, ielas,
      icmd, kk, l, nkt, icntrl, imag = 1, icomplex, kkv, kk6, iterm, lprev,
      ilength, ij, i1, i2, iel, ielset, node, indexe, nope, ml1, nelem,
      *inocs = NULL, *ielcs = NULL, jj, l1, l2, is, nlabel, *nshcon = NULL,
      ...
      nznode, ll, icfd = 0, *inomat = NULL, mortar = 0, *islavact = NULL,
      *ipobody = NULL, *islavnode = NULL, *nslavnode = NULL, *islavsurf = NULL,
      idirnew, *iponoel = NULL, *inoel = NULL, mscalmethod = 0, intscheme = 0,
      *islavquadel = NULL, *irowt = NULL, *jqt = NULL, mortartrafoflag = 0;

  long long lint;

  double *stn = NULL, *v = NULL, *temp_array = NULL, *vini = NULL,
         *csmass = NULL, *een = NULL, cam[5], *f = NULL, *fn = NULL, qa[4],
         *epn = NULL, summass, *stiini = NULL, *emn = NULL, *emeini = NULL,
         *clearini = NULL, *xstateini = NULL, theta, pi, *coefmpcnew = NULL,
         t[3], ctl, stl, *stx = NULL, *enern = NULL, *xstaten = NULL,
         *eei = NULL, *enerini = NULL, *qfx = NULL, *qfn = NULL, xreal, ximag,
         *vt = NULL, sum, *voldt = NULL, *coefright = NULL, coef, a[9], ratio,
         reltime, *physcon = NULL, *shcon = NULL, *springarea = NULL, *z = *zp,
         *zdof = NULL, *thicke = NULL, *sumi = NULL, *vti = NULL,
         *pslavsurf = NULL, *pmastsurf = NULL, *cdn = NULL, *energyini = NULL,
      ...
      if (ialset[i1] > 0) {
        ...
        if (*mcs == 1) {
          ...
          FORTRAN(calcmass, (ipkon, lakon, kon, co, mi, &nelem, ne, thicke,
                             ielmat, &nope, t0, rhcon, nrhcon, ntmat_, ithermal,
                             &csmass[i], ielprop, prop));
        }
        for (i2 = 0; i2 < nope; ++i2) {
  ...
  *nzdof = *nmddof;
  ...
  for (i = 0; i < *nload; i++) {
    ...
    iload = i + 1;
    FORTRAN(addizdofdload,
            (nelemload, sideload, ipkon, kon, lakon, nactdof, izdof, nzdof, mi,
             &iload, iznode, &nznode, nk, imdnode, nmdnode));
  }
  ...
  if (*nbody > 0) {
    printf(
        "\n *WARNING in expand: body loads are not allowed for modal "
        "dynamics\n          and steady state dynamics calculations in cyclic "
  ...
  while (i < *nforc) {
    ...
    i++;
    iforc = i;
    FORTRAN(addizdofcload,
            (nodeforc, ndirforc, nactdof, mi, izdof, nzdof, &iforc, iznode,
             &nznode, nk, imdnode, nmdnode, xforc, ntrans, inotr));
  }
  ...
  for (j = *nev - 1; j > -1; --j) {
    ...
    for (k = 0; k < 2 * neqh; k += neqh) {
      ...
      results(co, nk, kon, ipkon, lakon, ne, &v[kkv], &stn[kk6], inum, stx,
              elcon, nelcon, rhcon, nrhcon, alcon, nalcon, alzero, ielmat,
              ielorien, norien, orab, ntmat_, t0, t0, ithermal, prestr, iprestr,
              filab, eme, &emn[kk6], &een[kk6], iperturb, f, &fn[kkv], nactdof,
              &iout, qa, vold, &z[lint + k], nodeboun, ndirboun, xboun, nboun,
              ipompc, nodempc, coefmpcnew, labmpc, nmpc, nmethod, cam, &neqh,
              ...
              ener, &enern[kk], emeini, xstaten, eei, enerini, cocon, ncocon,
              set, nset, istartset, iendset, ialset, nprint, prlab, prset, qfx,
              qfn, trab, inotr, ntrans, fmpc, nelemload, nload, ikmpc, ilmpc,
              ...
              nener, orname, &network, ipobody, xbody, ibody, typeboun, itiefac,
    ...
    FORTRAN(rectcylexp, (co, v, fn, stn, qfn, een, cs, nk, &icntrl, t, filabt,
        ...
        for (ll = 0; ll < nznode; ll++) {
          ...
          if (inocs[l1] == jj) {
            ...
            for (l2 = 0; l2 < mt; l2++) {
              ...
              if (*nherm != 1)
                vti[l + mt * *nk * i] = stl * v[l] + ctl * v[l + mt * *nk];
            }
          }
        }
      }
    ...
    FORTRAN(rectcylexp,
            (co, vt, fn, stn, qfn, een, cs, &nkt, &icntrl, t, filabt, &imag, mi,
        ...
        for (k = 0; k < *nsectors; k++) {
          ...
          if (idof > -1) {
            ...
            if (id != 0) {
              if (izdof[id - 1] == idof) {
                if (*nherm == 1) {
                  zdof[(long long)j * *nzdof + id - 1] =
                      vt[k * mt * *nk + mt * i + j1];
                } else {
                  zdof[(long long)2 * j * *nzdof + id - 1] =
                      vt[k * mt * *nk + mt * i + j1];
                  zdof[(long long)(2 * j + 1) * *nzdof + id - 1] =
                      vti[k * mt * *nk + mt * i + j1];
                }
              }
            }
    ...
    for (i = 0; i < *mcs; i++) {
      if (nm[j] == 0 ||
          (nm[j] == (ITG)((cs[17 * i] / 2)) && (fmod(cs[17 * i], 2.) == 0.))) {
    ...
    if (*nherm == 1) {
      for (i = 0; i < *nzdof; i++) {
        zdof[(long long)j * *nzdof + i] /= sum;
      }
    } else {
      for (i = 0; i < *nzdof; i++) {
        zdof[(long long)(2 * j) * *nzdof + i] /= sum;
      }
      for (i = 0; i < *nzdof; i++) {
        zdof[(long long)(2 * j + 1) * *nzdof + i] /= sum;
      }
      sumi[j] = sqrt(sum);
    }
  }
  ...
  if (iperturb[0] == 1) {
    ...
    FORTRAN(rectcylvold,
            (co, vold, cs, &icntrl, mi, iznode, &nznode, nsectors, nk));
    ...
    FORTRAN(rectcylvold,
            (co, voldt, cs, &icntrl, mi, iznode, &nznode, nsectors, nk));
    ...
    for (j = 0; j < *nmpcold; j++) {
      ...
      if (strcmp1(&labmpcold[20 * j], "CYCLIC") == 0) {
        ...
        do {
          ...
          if ((itr != 0) && (idir != 0)) {
            if (trab[7 * itr - 1] < 0) {
              FORTRAN(transformatrix,
                      (&trab[7 * itr - 7], &co[3 * node - 3], a));
              if (iterm == 1) {
                for (k = 0; k < 3; k++) {
