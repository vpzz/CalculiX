...
static ITG *kon1, *ipkon1, *ne1, *nelcon1, *nrhcon1, *nalcon1, *ielmat1,
    *ielorien1, *norien1, *ntmat1_, *ithermal1, *iperturb1, *iout1, *nmethod1,
    *nplkcon1, *npmat1_, *mi1, *ncmat1_, *nstate1_, *ielprop1, *inoel1, *istep1,
    *iinc1, calcul_fn1, calcul_qa1, *nplicon1, *iponoel1,
    *nal = NULL, *ipompc1, *nodempc1, *nmpc1, *ncocon1, *ikmpc1, *ilmpc1,
    num_cpus, mt1, *nk1, *nshcon1, *nelemload1, *nload1, mortar1, *istartset1,
    *iendset1, *ialset1, *iactive1, *network1, *ipobody1, *ibody1,
    *neapar = NULL, *nebpar = NULL;

static double *co1, *v1, *elcon1, *rhcon1, *alcon1, *orab1, *t01, *eei1,
    *fn1 = NULL, *qa1 = NULL, *vold1, *dtime1, *time1, *prop1, *ttime1,
    *plkcon1, *xstateini1, *xstiff1, *xstate1, *sti1, *springarea1, *reltime1,
    *coefmpc1, *vini1, *cocon1, *qfx1, *shcon1, *xload1, *plicon1, *xloadold1,
    *h01, *pslavsurf1, *pmastsurf1, *clearini1, *xbody1;
...
void resultsinduction(
    ...
    double *coefmpc, char *labmpc, ITG *nmpc, ITG *nmethod, double *cam,
    ITG *neq, double *veold, double *accold, double *bet, double *gam,
  ...
  FORTRAN(resultsini_em,
          (nk, v, ithermal, filab, iperturb, f, fn, nactdof, iout, qa, b,
           nodeboun, ndirboun, xboun, nboun, ipompc, nodempc, coefmpc, labmpc,
           nmpc, nmethod, cam, neq, veold, dtime, mi, vini, nprint, prlab,
  ...
  if (((ithermal[0] <= 1) || (ithermal[0] >= 3)) && (intpointvarm == 1)) {
    ...
    if (((*nmethod != 4) && (*nmethod != 5)) || (iperturb[0] > 1)) {
      printf(" Using up to %" ITGFORMAT
             " cpu(s) for the magnetic field calculation.\n\n",
             num_cpus);
    }
  ...
  if ((ithermal[0] >= 2) && (intpointvart == 1)) {
    ...
    for (i = 0; i < num_cpus; i++) {
      ithread[i] = i;
      pthread_create(&tid[i], NULL, (void *)resultsthermemmt,
                     (void *)&ithread[i]);
    }
    for (i = 0; i < num_cpus; i++) pthread_join(tid[i], NULL);
  ...
  FORTRAN(resultsforc_em, (nk, f, fn, nactdof, ipompc, nodempc, coefmpc, labmpc,
                           nmpc, mi, fmpc, &calcul_fn, &calcul_f, inomat));
  ...
  FORTRAN(
      resultsprint,
      (co, nk, kon, ipkon, lakon, ne, v, stn, inum, sti, ielorien, norien, orab,
       t1, ithermal, filab, een, iperturb, fn, nactdof, iout, vold, nodeboun,
       ndirboun, nboun, nmethod, ttime, xstate, epn, mi, nstate_, ener, enern,
       xstaten, eei, set, nset, istartset, iendset, ialset, nprint, prlab,
       prset, qfx, qfn, trab, inotr, ntrans, nelemload, nload, &ikin, ielmat,
       thicke, eme, emn, rhcon, nrhcon, shcon, nshcon, cocon, ncocon, ntmat_,
       sideload, icfd, inomat, pslavsurf, islavact, cdn, &mortar, islavnode,
       nslavnode, ntie, islavsurf, time, ielprop, prop, veold, ne0, nmpc,
       ipompc, nodempc, labmpc, energyini, energy, orname, xload, itiefac,
       pmastsurf, springarea, tieset, ipobody, ibody, xbody, nbody, iinc));
...
void *resultsemmt(ITG *i) {
  ...
  FORTRAN(resultsem, (co1, kon1, ipkon1, lakon1, v1, elcon1, nelcon1, ielmat1,
                      ntmat1_, vini1, dtime1, matname1, mi1, ncmat1_, &nea,
                      &neb, sti1, alcon1, nalcon1, h01, istartset1, iendset1,
                      ialset1, iactive1, fn1, eei1, iout1, nmethod1));
...
void *resultsthermemmt(ITG *i) {
  ...
  FORTRAN(
      ...
      (co1, kon1, ipkon1, lakon1, v1, elcon1, nelcon1, rhcon1, nrhcon1, ielmat1,
       ...
       &mortar1, clearini1, plicon1, nplicon1, ielprop1, prop1, iponoel1,
       inoel1, network1, ipobody1, xbody1, ibody1));
