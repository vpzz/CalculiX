...
static ITG *nstate1_, *mi1, *islavsurf1, *ne01, *islavsurfold1, numstart,
    numfaces, num_cpus;
...
void interpolatestatemain(ITG *ne, ITG *ipkon, ITG *kon, char *lakon, ITG *ne0,
  ...
  for (i = 0; i < *ntie; i++) {
    ...
    nstate1_ = nstate_;
    mi1 = mi;
    islavsurf1 = islavsurf;
    pslavsurf1 = pslavsurf;
    ne01 = ne0;
    islavsurfold1 = islavsurfold;
    pslavsurfold1 = pslavsurfold;
...
void *interpolatestatemainmt(ITG *i) {
  ...
  for (kk = nfaa; kk <= nfab; kk++) {
    ...
    if (numpts > 2) {
      FORTRAN(interpolateinface,
              (&kk, xstate1, xstateini1, &numpts, nstate1_, mi1, islavsurf1,
               pslavsurf1, ne01, islavsurfold1, pslavsurfold1));
    }
  }
