...
void refinemesh(ITG *nk, ITG *ne, double *co, ITG *ipkon, ITG *kon, double *v,
  ...
  ITG nktet_, netet_,
      *kontet = NULL, *ifatet = NULL, *ifac = NULL, *itetfa = NULL,
      *ipofa = NULL, *ipoeln = NULL, *ieln = NULL, ifreetet = 1, ifreefa = 1,
      ielem, ifreeln = 1, *iedg = NULL, *ipoed = NULL, *ipoeled = NULL,
      *ieled = NULL, *iedtet = NULL, ifreeed = 1, ifreele = 1,
      *integerglob = NULL, *iexternfa = NULL, *iexternedg = NULL,
      *iexternnode = NULL, *n = NULL, nnewnodes, newsize, *ibasenewnodes = NULL,
      i, k, *irand1 = NULL, *irand2 = NULL, iext, iedge, kflag, node, ibase,
      nktet, *nh = NULL, ij, nloop, *iedgnewnodes = NULL, *kontetor = NULL,
      iquad = 0, *iedgext = NULL, nexternedg, *iedgmid = NULL, *ifacext = NULL,
      nexternfa, *iexternfaor = NULL, *itreated = NULL, *ialsete = NULL,
      nexternel, *iedgextfa = NULL, *ifacexted = NULL, *ilist = NULL,
      *isharp = NULL, *idimsh = NULL, ifreenn = 1, *iponn = NULL, *inn = NULL,
      *n1newnodes = NULL, *n2newnodes = NULL, *jfix = NULL, *number = NULL,
  ...
  double *cotet = NULL, *planfa = NULL, *bc = NULL, *cg = NULL, *h = NULL, dcg,
         ...
         *quality = NULL, randomnumber, *conewnodesorig = NULL, cotetorig[3],
         height, c1, *hnewnodes = NULL, *qualityjac = NULL;
  ...
  FORTRAN(cattet, (kontet, &netet_, ifac, ne, ipkon, kon, ifatet, &ifreetet, bc,
                   itetfa, &ifreefa, planfa, ipofa, cotet, cg, ipoeln, ieln,
                   &ifreeln, lakon, kontetor, &iquad, istartset, iendset,
  ...
  FORTRAN(determineextern, (ifac, itetfa, iedg, ipoed, iexternedg, iexternfa,
                            iexternnode, &nktet_, ipofa));
  ...
  FORTRAN(midexternaledges, (iexternedg, &nexternedg, iedgext, &ifreeed, ieled,
                             ipoeled, iedg, iedtet, kontetor));
  ...
  FORTRAN(midexternalfaces, (iexternfa, &nexternfa, ifacext, &ifreefa, itetfa,
                             ifac, kontet, kontetor, ialsete, &nexternel,
                             iedgextfa, ifacexted, ipoed, iedg, iexternedg));
  ...
  for (ij = 0; ij < nloop; ij++) {
    ...
    if (ij == 0) {
      NNEW(nh, ITG, nktet_);
      FORTRAN(calculateh,
              (nk, v, veold, stn, een, emn, epn, enern, qfn, errn, h, filab, mi,
               d, nh, &dmin, ipoed, iedg, cotet, jfix));
      SFREE(nh);
      ...
      FORTRAN(updategeodata,
              (&nktet, &netet_, h, d, &dmin, ipoed, iedg, cotet, planfa, bc, cg,
               kontet, ifac, ipofa, doubleglob, integerglob, ipoeln));
    }
    ...
    FORTRAN(edgedivide, (&nnewnodes, &nktet_, ipoed, iexternedg, iedg, d, h, n,
                         r, &iext, jfix));
    ...
    FORTRAN(newnodes,
            (&nktet_, ipoed, n, iedg, h, d, r, conewnodes, cotet, ibasenewnodes,
             ipoeled, ieled, doubleglob, integerglob, &nnewnodes, iedgnewnodes,
             hnewnodes, n1newnodes, n2newnodes));
    ...
    for (i = 0; i < nnewnodes; i++) {
      ...
      FORTRAN(cavityext_refine,
              (kontet, ifatet, &ifreetet, bc, ifac, itetfa, &ifreefa, planfa,
               ipofa, cotet, &ibase, &node, iexternfa, ipoed, iedg, &ifreeed,
               ipoeled, ieled, &ifreele, &nktet, &netet_, ibasenewnodes,
               conewnodes, ipoeln, ieln, &ifreeln, &nnewnodes, iexternedg,
               iedtet, cotetorig, &iedge, iexternnode, cg, &height, iparentel));
      ...
      if (ifreetet > netet_ - 1000) {
        ...
        FORTRAN(reinit_refine, (kontet, ifac, ieln, &netet_, &newsize, ifatet,
                                itetfa, iedg, ieled));
        netet_ = newsize;
      }
    ...
    for (i = 0; i < nnewnodes; i++) {
      ...
      FORTRAN(cavity_refine,
              (kontet, ifatet, &ifreetet, bc, ifac, itetfa, &ifreefa, planfa,
               ipofa, cotet, &ibase, &node, iexternfa, ipoed, iedg, &ifreeed,
               ipoeled, ieled, &ifreele, &nktet, &netet_, ibasenewnodes,
               conewnodes, ipoeln, ieln, &ifreeln, &nnewnodes, iexternedg,
               iedtet, cg, &height, iparentel));
    ...
    FORTRAN(smoothingvertexnodes,
            (inn, iponn, &nktet, iexternnode, &netet_, kontet, cotet, ipoeln,
             ieln, h, quality, jfix));
  ...
  FORTRAN(
      projectvertexnodes,
      (ipoed, iexternedg, iedgext, cotet, &nktet, iedg, iexternfa, ifacext,
       itreated, ilist, isharp, ipofa, ifac, iedgextfa, ifacexted, co, idimsh,
       ipoeln, ieln, kontet, &c1, &jflag, ibadnodes, &nbadnodes, &iwrite));

  /* optimizing the position of subsurface neighbors of vertices,
     which were not fully projected */

  FORTRAN(smoothbadvertex,
          (cotet, kontet, ipoeln, ieln, &nbadnodes, ibadnodes, iponn, inn,
           iexternnode, ipoeled, ieled, iedgmid, iedtet));
  ...
  FORTRAN(removesliver, (&netet_, kontet, iexternnode, iedtet, iexternedg,
                         quality, itetfa, ipofa, ipoeln, ipoeled, ipoed,
                         &ifreetet, &ifreeln, &ifreele, &ifreefa, &ifreeed,
                         ifatet, ifac, iexternfa, ieln, ieled, iedg, isharp));
  ...
  if (iquad == 1) {
    ...
    FORTRAN(genmidnodes,
            (&nktet_, ipoed, iedgmid, iexternedg, iedgext, cotet, &nktet, iedg,
             jfix, ipoeled, ieled, kontet, iedtet, &iwrite));
    ...
    FORTRAN(searchmidneigh, (inn, iponn, &nktet_, iexternedg, ipoed, iedg,
                             ipoeled, ieled, &ifreenn, iedgmid, iedtet));
    ...
    for (ij = 0; ij < 5; ij++) {
      ...
      c1 = 0.5 + 0.1 * (ij + 1);
      if (ij == 4) jflag = 1;
      FORTRAN(projectmidnodes,
              (&nktet_, ipoed, iedgmid, iexternedg, iedgext, cotet, &nktet,
               iedg, iexternfa, ifacext, itreated, ilist, isharp, ipofa, ifac,
               iedgextfa, ifacexted, jfix, co, idimsh, ipoeled, ieled, kontet,
               &c1, &jflag, iedtet, ibadnodes, &nbadnodes, &iwrite));

      FORTRAN(smoothbadmid, (cotet, kontet, ipoeln, ieln, &nbadnodes, ibadnodes,
                             iexternedg, ipoeled, ieled, iedgmid, iedtet));

      if (ij < 4) {
        ielem = 0;
        FORTRAN(quadmeshquality,
                (&netet_, cotet, kontet, iedtet, iedgmid, qualityjac, &ielem));

        FORTRAN(smoothingmidnodes,
                (cotet, ipoed, kontet, iedtet, iedgmid, ipoeled, ieled,
                 qualityjac, iponn, inn, h, iexternedg, &netet_, &nktet_));
      }
    }
  }
  ...
  FORTRAN(writerefinemesh,
          (kontet, &netet_, cotet, &nktet, jobnamec, &iquad, iedtet, iedgmid,
