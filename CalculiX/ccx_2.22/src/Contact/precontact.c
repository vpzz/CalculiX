...
void precontact(ITG *ncont, ITG *ntie, char *tieset, ITG *nset, char *set,
  ...
  ITG i, j, ntrimax, *nx = NULL, *ny = NULL, *nz = NULL, im, l, nstart, kflag,
                     ntri, ii;
  ...
  FORTRAN(updatecontpen,
          (koncont, ncont, co, vold, cg, straight, mi, imastnode, nmastnode,
           xmastnor, ntie, tieset, nset, set, istartset, iendset, ialset, ipkon,
  ...
  if ((*istep == 1) && (*iinc == 1) && (*iit <= 0)) {
    ...
    FORTRAN(adjustcontactnodes,
            (tieset, ntie, itietri, cg, straight, co, vold, xo, yo, zo, x, y, z,
             nx, ny, nz, istep, iinc, iit, mi, imastop, nslavnode, islavnode,
             set, nset, istartset, iendset, ialset, tietol, clearini,
             clearslavnode, itiefac, ipkon, kon, lakon, islavsurf));
  ...
  for (i = 0; i < *ntie; i++) {
    ...
    if (tieset[i * (81 * 3) + 80] == 'C') {
      ...
      for (l = itiefac[2 * i]; l <= itiefac[2 * i + 1]; l++) {
        ...
        FORTRAN(slavintpoints,
                (ntie, itietri, ipkon, kon, lakon, straight, nintpoint, koncont,
                 co, vold, xo, yo, zo, x, y, z, nx, ny, nz, islavsurf,
                 islavnode, nslavnode, imastop, mi, ncont, ipe, ime, pslavsurf,
                 &ii, &l, &ntri));
      }
    }
  }
