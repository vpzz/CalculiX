...
void buildtquad(ITG *ntie, ITG *ipkon, ITG *kon, ITG *nk, char *lakon,
  ...
  ITG i, j, l, nodesf, nodem, istart, icounter, ndim, ifree, ifree2, nzstloc,
      nzstlocinv, *krow = NULL, *kcol = NULL, *mast1 = NULL, *mast2 = NULL,
                  *irowt = NULL, *irowtinv = NULL;
  ...
  for (i = 0; i < *ntie; i++) {
    if (tieset[i * (81 * 3) + 80] == 'C') {
      for (l = itiefac[2 * i]; l <= itiefac[2 * i + 1]; l++) {
        ...
        FORTRAN(create_t, (ipkon, kon, lakon, islavsurf, contr, krow, kcol,
                           &icounter, &l));
        ...
        FORTRAN(create_tinv, (ipkon, kon, lakon, islavsurf, contr, krow, kcol,
                              &icounter, &l));
