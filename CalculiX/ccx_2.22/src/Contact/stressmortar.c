...
/**  transforming the system back to the standard basis functions
    ...
    *  [in] 	gap		(i) gap for slave node i
     *  [in,out] b		in: differenzial displacement out:real
   displacement
     *  [in,out] islavact	(i) indicates, if slave node i is active (=-3
   no-slave-node, =-2 no-LM-node, =-1 no-gap-node, =0 inactive node, =1 sticky
   node, =2 slipping/active node)
     *  [out] irowddinv	field containing row numbers of auddinv
     *  [out] jqddinv		pointer into field irowddinv
     *  [out] auddinv		coupling matrix \f$
   \tilde{D}^{-1}_d[nactdof(i,p),nactdof(j,q)]\f$ for all active degrees od
   freedoms
     *  [in] irowt		field containing row numbers of aut
     *  [in] jqt	        pointer into field irowt
     *  [in] aut		transformation matrix \f$ T[p,q]\f$ for slave
   nodes \f$ p,q \f$
     *  [in] irowtinv	field containing row numbers of autinv
     *  [in] jqtinv	pointer into field irowtinv
     *  [in] autinv	transformation matrix \f$ T^{-1}[p,q]\f$ for slave nodes
   \f$ p,q \f$
     *  [in] slavnor		slave normals
     *  [in] slavtan		slave tangents
     *  [out]	iflagact	flag indicating if semi-smooth Newton has
   converged
     ...
     *  [out] Bd		coupling matrix \f$ B_d[p,q]=\int \psi_p \phi_q
   dS \f$, \f$ p \in S, q \in M \f$
     *  [out] irowb		field containing row numbers of Bd
     *  [out] jqb		pointer into field irowb
     *  [out] Dd		coupling matrix \f$ D_d[p,q]=\int \psi_p \phi_q
   dS \f$, \f$ p,q \in S \f$
     *  [out] irowd		field containing row numbers of Dd
     *  [out] jqd		pointer into field irowd
     *  [out] Ddtil		coupling matrix \f$ \tilde{D}_d[p,q]=\int \psi_p
   \tilde{\phi}_q dS \f$, \f$ p,q \in S \f$
...
void stressmortar(
  ...
  ITG i, j, l, jj, k, idof1, idof2, idof3, nodes,
      mt = mi[1] + 1, nstick = 0, nslip = 0, ninacti = 0, nnogap = 0, nolm = 0,
      ndiverg, nhelp, idof, node2, dirind, dirdep, index, ist, iout,
      num_cpus = 1, keepset, derivmode, regmode, ndof, calcul_fn, calcul_f;

  double aux, stressnormal, ddispnormal,
      *unitmatrix = NULL, constant = 1.E10, constantn = 1.E10,
      constantt = 1.E10, stresst[2], stressinit[2], stresstildet[2],
      disp_tildet[2], nw_t = 0.0, *du = NULL, ch, coefdep, disp_t[2], scal,
      w_t[3], *bhat2 = NULL, *fmpc = NULL, lm_t1_av, lm_t2_av, *rc = NULL,
      f_cs_tot[4], f_cm_tot[4], *vectornull = NULL, *u_oldt = NULL,
      *cstress2 = NULL, *cstresstil = NULL, *cstressini2 = NULL, *u_old = NULL,
      aninvloc, atauinvloc, gnc, gtc[2], *cold = NULL, *cold2 = NULL, ln_old,
      ncf_n, max_ncf_n, ncf_t[2], max_ncf_t[2], mu, mumax, p0, beta, *b2 = NULL,
      alpha;
  ...
  *iflagact = 0;
  ...
  for (i = 0; i < neq[1]; i++) {
    f_cs[i] = bhat[i] - vectornull[i];
  }

  for (i = 0; i < neq[1]; i++) {
    vectornull[i] = 0.0;
  }
  FORTRAN(opnonsymt, (&neq[1], &aux, f_cs, cstress2, vectornull, auddinv,
                      jqddinv, irowddinv));
  for (i = 0; i < neq[1]; i++) {
    f_cs[i] = 0.0;
  }
  ...
  FORTRAN(resultsini_mortar,
          (nk, b2, ithermal, iperturb, nactdof, &iout, vold, b, nodeboun,
           ndirboun, xboun, nboun, ipompc, nodempc, coefmpc, labmpc, nmpc,
           nmethod, cam, bet, gam, dtime, mi));
  ...
  for (i = 0; i < *nk; i++) {
    ...
    for (jj = jqd[nodes - 1] - 1; jj < jqd[nodes - 1 + 1] - 1; jj++) {
      for (k = 0; k < 3; k++) {
        du[(islavnodeinv[irowd[jj] - 1] - 1) * 3 + k] +=
            Dd[jj] * b2[mt * nodes - 3 + k];
  ...
  for (i = 0; i < *nk; i++) {
    ...
    for (jj = jqb[nodes - 1] - 1; jj < jqb[nodes - 1 + 1] - 1; jj++) {
      for (k = 0; k < 3; k++) {
        du[(islavnodeinv[irowb[jj] - 1] - 1) * 3 + k] +=
            Bd[jj] * b2[mt * nodes - 3 + k];
  ...
  for (i = 0; i < *ntie; i++) {
    if (tieset[i * (81 * 3) + 80] == 'C') {
      for (j = nslavnode[i]; j < nslavnode[i + 1]; j++) {
        ...
        FORTRAN(getcontactparams, (&mu, &regmode, &aninvloc, &atauinvloc, &p0,
                                   &beta, tietol, elcon, &i, ncmat_, ntmat_));
        mumax = max(mu, mumax);
        stressnormal = cstresstil[mt * j + 0] * slavnor[3 * j] +
                       cstresstil[mt * j + 1] * slavnor[3 * j + 1] +
                       cstresstil[mt * j + 2] * slavnor[3 * j + 2];
        stresst[0] = cstresstil[mt * j + 0] * slavtan[6 * j] +
                     cstresstil[mt * j + 1] * slavtan[6 * j + 1] +
                     cstresstil[mt * j + 2] * slavtan[6 * j + 2];
        stresst[1] = cstresstil[mt * j + 0] * slavtan[6 * j + 3] +
                     cstresstil[mt * j + 1] * slavtan[6 * j + 4] +
                     cstresstil[mt * j + 2] * slavtan[6 * j + 5];
        ...
        ddispnormal = du[3 * j + 0] * slavnor[3 * j] +
                      du[3 * j + 1] * slavnor[3 * j + 1] +
                      du[3 * j + 2] * slavnor[3 * j + 2];
        disp_tildet[0] =
            (du[3 * j + 0] + u_oldt[j * 3]) * slavtan[6 * j] +
            (du[3 * j + 1] + u_oldt[j * 3 + 1]) * slavtan[6 * j + 1] +
            (du[3 * j + 2] + u_oldt[j * 3 + 2]) * slavtan[6 * j + 2];
        disp_tildet[1] =
            (du[3 * j + 0] + u_oldt[j * 3]) * slavtan[6 * j + 3] +
        ...
        ln_old = cold2[3 * j + 0] * slavnor[3 * j] +
                 cold2[3 * j + 1] * slavnor[3 * j + 1] +
                 cold2[3 * j + 2] * slavnor[3 * j + 2];

        double gnc_old, dgnc_old;
        derivmode = 0;
        if (islavact[j] > -1) {
          scal = Ddtil[jqdtil[nodes - 1] - 1];
        } else {
          scal = 0.0;
        }
        FORTRAN(regularization_gn_c,
                (&ln_old, &derivmode, &regmode, &gnc_old, &aninvloc, &p0, &beta,
                 elcon, nelcon, &i, ntmat_, plicon, nplicon, npmat_, ncmat_,
                 tietol, &scal));
        derivmode = 1;
        FORTRAN(regularization_gn_c,
                (&ln_old, &derivmode, &regmode, &dgnc_old, &aninvloc, &p0,
                 &beta, elcon, nelcon, &i, ntmat_, plicon, nplicon, npmat_,
                 ncmat_, tietol, &scal));
        derivmode = 0;
        FORTRAN(regularization_gn_c,
                (&stressnormal, &derivmode, &regmode, &gnc, &aninvloc, &p0,
                 &beta, elcon, nelcon, &i, ntmat_, plicon, nplicon, npmat_,
                 ncmat_, tietol, &scal));

        derivmode = 0;
        FORTRAN(regularization_gt_c,
                (stresstildet, &derivmode, gtc, &atauinvloc));
        ...
        if (mu > 1.E-10) {
          bp[j] =
              mu * (stressnormal + constantn * (ddispnormal - gap[j] - gnc));
        } else {
        ...
        if (ncf_n > max_ncf_n && ndof > 0 && islavact[j] > -1) {
          max_ncf_n = ncf_n;
        }
  ...
  if (keepset == 0) {
    ...
    if ((max_ncf_t[0] / ((lm_t1_av + 0.001) / (nstick + nslip + 0.001)) >
             9.e-4 ||
  ...
  for (i = 0; i < *nk; i++) {
    for (j = jqd[i] - 1; j < jqd[i + 1] - 1; j++) {
      for (l = 0; l < 3; l++) {
        cfs[mt * (i + 1) - 3 + l] +=
            Dd[j] * cstress[mt * (islavnodeinv[irowd[j] - 1] - 1) + l];
      }
    }
  }
  for (i = 0; i < *nk; i++) {
    for (j = jqb[i] - 1; j < jqb[i + 1] - 1; j++) {
      for (l = 0; l < 3; l++) {
        cfs[mt * (i + 1) - 3 + l] +=
            Bd[j] * cstress[mt * (islavnodeinv[irowb[j] - 1] - 1) + l];
      }
    }
  }
  if (*nmethod == 4) {
    for (i = 0; i < *nk; i++) {
      for (j = jqdtil[i] - 1; j < jqdtil[i + 1] - 1; j++) {
        for (l = 0; l < 3; l++) {
          cfstil[mt * (i + 1) - 3 + l] +=
              Ddtil[j] * cstress[mt * (islavnodeinv[irowdtil[j] - 1] - 1) + l];
        }
      }
    }
    for (i = 0; i < *nk; i++) {
      for (j = jqbtil[i] - 1; j < jqbtil[i + 1] - 1; j++) {
        for (l = 0; l < 3; l++) {
          cfstil[mt * (i + 1) - 3 + l] +=
              Bdtil[j] * cstress[mt * (islavnodeinv[irowbtil[j] - 1] - 1) + l];
        }
      }
    }
  ...
  for (i = 0; i < *nk; i++) {
    for (l = 0; l < 3; l++) {
      if (*nmethod == 4) {
        ...
        rc[mt * (i + 1) - 3 + l] = cfs[mt * (i + 1) - 3 + l] * (1 + alpha) -
                                   cfsini[mt * (i + 1) - 3 + l] * (alpha);
      } else {
  ...
  resultsforc(nk, f_cs, rc, nactdof, ipompc, nodempc, coefmpc, labmpc, nmpc, mi,
