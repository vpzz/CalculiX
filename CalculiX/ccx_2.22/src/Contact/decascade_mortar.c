...
void decascade_mortar(ITG *nmpc, ITG *ipompc, ITG **nodempcp, double **coefmpcp,
  ...
  ITG i, j, k, l, nl, index, indexold, index1, index2, index2old, indexnew,
      index1old, iexpand, ichange, idof, id, mpc, *nodempc = NULL;
  ...
  for (i = 0; i < *nmpc; i++) {
    ...
    if (index == 0) continue;
    do {
      // if(i+1==1060||i+1==1061)printf("\tindex %"ITGFORMAT" node %"ITGFORMAT"
          ...
          if (indexnew != 0) {
            if (*mpcfree == 0) {
              ...
              *memmpc_ = (ITG)(1.1 * *memmpc_);
              // printf("*INFO in cascade: reallocating nodempc; new size = %"
              // ITGFORMAT "\n\n",*memmpc_);
            ...
            // if(i+1==1060||i+1==1061)printf("\t\tindex %"ITGFORMAT" node
            // %"ITGFORMAT" dir %"ITGFORMAT" nindex %"ITGFORMAT"
