...
void massless(ITG *kslav, ITG *lslav, ITG *ktot, ITG *ltot, double *au,
              ...
              ITG *neqtot, double *qb, double *b, double *dtime, double *aloc,
  ...
  ITG *jqwnew = NULL, *irowwnew = NULL, symmetryflag = 0, mt = mi[1] + 1, im,
      ...
      *jqbb = NULL, *irowbb = NULL, *icolbb = NULL, nzsbb, *jqbi = NULL,
      *irowbi = NULL, nzsbi, *jqib = NULL, *irowib = NULL, nzsib, nrhs = 1,
      neqslavs, *ithread = NULL;

  double *auwnew = NULL, sigma = 0.0, *gapdisp0 = NULL, *gapnorm = NULL,
         *cvec = NULL, sum, *adbbb = NULL, *aubbb = NULL, *gvec = NULL,
         *gmatrix = NULL, *qi_kbi = NULL, *veolddof = NULL, atol, rtol,
         *aubb = NULL, *adbb = NULL, *gapdisp = NULL, *al = NULL, *alnew = NULL,
         *r = NULL, *rhs = NULL, *aubi = NULL, *auib = NULL, omega,
         *alocold = NULL, *ddisp = NULL;
  ...
  if ((*masslesslinear == 0) || (*iinc == 1)) {
    ...
    FORTRAN(extract_matrices, (au, ad, jq, irow, neq, aubb, adbb, jqbb, irowbb,
                               neqtot, &nzsbb, aubi, jqbi, irowbi, &nzsbi, auib,
                               jqib, irowib, &nzsib, ktot, icolbb));
...
#ifdef PARDISO
      pardiso_factor_cp(adbb, aubb, adbbb, aubbb, &sigma, icolbb, irowbb,
                        neqtot, &nzsbb, &symmetryflag, &inputformat, jqbb,
                        &nzsbb, iexpl);
#else
...
#ifdef PASTIX
      ITG inputformat = 1;
      pastix_factor_main_cp(adbb, aubb, adbbb, aubbb, &sigma, icolbb, irowbb,
                            neqtot, &nzsbb, &symmetryflag, &inputformat, jqbb,
                            &nzsbb);
#else
  ...
  FORTRAN(detectactivecont, (gapnorm, gapdisp0, auw, iroww, jqw, nslavs,
                             springarea, iacti, &nacti));
  ...
  if (nacti > 0) {
    ...
    for (i = 0; i < *neqtot; i++) {
      gapdisp0[i] =
          (gapdisp0[i] - volddof[ktot[i] - 1]) / (*dtime);  // gapdisp0 - qb_km1
    }
    ...
    //  For full FE it should play a big role in performance, unless the total
    //  of contact DOF is very high. We control this parameter with OMEGA.
    //  Recommended is 100% (no modification) No more than 200% or less than
    //  100%. should not be necesary to decrease it < 100%
    ...
    FORTRAN(inclusion, (gmatrix, cvec, iacti, &nacti, fric, &atol, &rtol,
                        alglob, &kitermax, auw, jqw, iroww, nslavs, al, alnew,
                        r, &omega, masslesslinear, fullr));
  ...
  if (*nener == 1) {
    ...
    for (i = 0; i < neqslavs; i++) {
      ...
      if (3 * (i / 3) != i) {
        ...
        if (iacti[i] != 0) {
          for (j = jqw[i] - 1; j < jqw[i + 1] - 1; j++) {
            ddisp[i] +=
                auw[j] * (qb[iroww[j] - 1] - volddof[ktot[iroww[j] - 1] - 1]);
          }
        }
      }
    }
    ...
    for (i = 0; i < neqslavs; i++) {
      if (3 * (i / 3) != i) {
        if (iacti[i] != 0) {
          ener[2 * mi[0] * (*ne + i / 3) + 1] +=
              -(alocold[i] + aloc[i]) * ddisp[i] / 2.;
        }
      }
    }
    SFREE(alocold);
  ...
  itranspose = 1;
  mulmatvec_asymmain(aubi, jqbi, irowbi, &neq[0], qb, rhs, &itranspose,
                     &neq[0]);
