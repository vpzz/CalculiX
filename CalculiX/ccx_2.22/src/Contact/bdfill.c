...
/*
  Calculates the coupling matrices \f$ B_d \f$ and \f$ D_d \f$, and insert them
...
void bdfill(ITG **irowbdp, ITG *jqbd, double **aubdp, ITG *nzsbd,
  ...
  ITG i, ii, j, jj, k, kk, l, ll, icounter, icounter2, idof1, idofs, idofm,
      nodesf, nodem, isn, imn,
      *mast1 = NULL, nzsbhelp, idof0, *irowbd = NULL, *irowdd = NULL,
      *irowddinv = NULL, *irowddtil = NULL, *irowbdtil = NULL,
      *irowbdtil2 = NULL, *irowddtil2 = NULL, ifree, ifree1, ifree3,
      mt = mi[1] + 1, istart, *iscontr = NULL, *imcontr = NULL, *igap = NULL,
      *idcontr1 = NULL, *idcontr2 = NULL, *igcontr = NULL, intpointl,
      *mast2 = NULL, ifree2, ist, ist2, node1, dim, index, dirdep, dirind,
      index2, dirdep2, dirind2, node2, idof2, idof3, nzsddinv, nzsdd,
      *irowb = NULL, nzsb1, nzsbtil2, nzsdtil, nzsd, nzsb, nzsdinv, nzsbtil,
      nzsd1, nzsb2a, *mast3 = NULL, *irowdtil = NULL, *irowb1 = NULL,
      *jqb1 = NULL, *irowb2 = NULL, *jqb2 = NULL, *irowbtil2 = NULL,
      *jqbtil2 = NULL, *irowd = NULL, *irowbtil = NULL, *irowdinv = NULL,
      *jqdinv = NULL, *jqb2a = NULL, *irowb2a = NULL, *jqd1 = NULL,
      *irowd1 = NULL, *irowbhelp = NULL, *irowbtil2t = NULL, *jqbtil2t = NULL,
      *irowdtil2 = NULL, *jqdtil2 = NULL, nzsddtil2, nzsdtil2, islavnodeentry,
      jrow, iadd;

  double contribution = 0.0, *aubd = NULL, *audd = NULL, *auddtil = NULL,
         *auddtil2 = NULL, *auddinv = NULL, *Bdhelp = NULL, *contr = NULL,
         *dcontr = NULL, *gcontr = NULL, *Bd = NULL, *aubdtil = NULL,
         *aubdtil2 = NULL, c2, coefdep, coefdep2, c3, *Dd = NULL, *Ddinv = NULL,
         *Ddtil2 = NULL, *dinvloc = NULL, *dloc = NULL, *Bd1 = NULL,
         *Bd2 = NULL, *Bdtil2 = NULL, *Bdtil = NULL, *Ddtil = NULL, *Dd1 = NULL,
         *Bd2a = NULL, *Bdtil2t = NULL, detdloc, e1, e2, e3;
  ...
  */
    ...
    if (tieset[i * (81 * 3) + 80] == 'C') {
      ...
      for (l = itiefac[2 * i]; l <= itiefac[2 * i + 1]; l++) {
        ...
        if (intpointl > 0) {
          ...
          FORTRAN(createbd,
                  (&i, &l, ipkon, kon, lakon, co, vold, gapmints, islavsurf,
                   imastsurf, pmastsurf, contr, iscontr, imcontr, dcontr,
                   idcontr1, idcontr2, gcontr, igcontr, mi, pslavsurf,
                   pslavdual, nslavnode, islavnode, nmastnode, imastnode,
          ...
          for (j = 0; j < icounter2; j++) {
            ...
            if (nodesf == nodem) {
              ...
              if (contribution > 1e-14 || contribution < -1e-14) {
                ...
                insertas(&irowbtil2, &mast3, &nodem, &nodesf, &ifree3,
                         &nzsbtil2, &contribution, &Bdtil2);
              }
            }
          }
  ...
  NNEW(jqb2a, ITG, *nk + 1);
  multi_rect(Bdtil2, irowbtil2, jqbtil2, dim, dim, autinv, irowtinv, jqtinv,
             dim, dim, &Bd2a, &irowb2a, jqb2a, &nzsb2a);

  NNEW(Bdtil2t, double, nzsbtil2);
  NNEW(irowbtil2t, ITG, nzsbtil2);
  NNEW(jqbtil2t, ITG, *nk + 1);
  dim = *nk;
  transpose(Bdtil2, jqbtil2, irowbtil2, &dim, Bdtil2t, jqbtil2t, irowbtil2t);

  nzsd = nzsd1;
  RENEW(Dd, double, nzsd);
  RENEW(irowd, ITG, nzsd);
  add_rect(Dd1, irowd1, jqd1, dim, dim, Bd2a, irowb2a, jqb2a, dim, dim, &Dd,
           &irowd, jqd, &nzsd);
  ...
  NNEW(irowb2, ITG, 1);
  NNEW(jqb2, ITG, *nk + 1);
  add_rect(Bd1, irowb1, jqb1, dim, dim, Bd2, irowb2, jqb2, dim, dim, &Bd,
           &irowb, jqb, &nzsb);
  ...
  for (i = 0; i < *nk; i++) {
    if (jqdtil[i + 1] - jqdtil[i] == 1) {
      ...
      nodem = i + 1;
      insertas_ws(&irowdinv, &nodesf, &nodem, &ifree2, &nzsdinv, &contribution,
                  &Ddinv);
    } else {
      // something went wrong
    }
  ...
  add_rect(Bd1, irowb1, jqb1, dim, dim, Bdtil2t, irowbtil2t, jqbtil2t, dim, dim,
  ...
  for (i = 0; i < *nk; i++) {
    for (j = jqd[i] - 1; j < jqd[i + 1] - 1; j++) {
      ...
      for (ll = k; ll < 3; ll++) {
        ...
        if ((idofs > 0) && (idofm > 0) &&
            (contribution > 1e-18 || contribution < -1e-18)) {
          ...
          insertas(&irowdd, &mast1, &idofs, &idofm, &ifree, &nzsdd,
                   ...
                   (contribution > 1e-18 || contribution < -1e-18)) {
          // mpc on master node
          for (jj = nslavmpc[2 * (imn - 1)]; jj < nslavmpc[2 * (imn - 1) + 1];
               jj++) {
            ist = islavmpc[jj];
            ...
            if (ll == (dirdep - 1)) {
              while (index != 0) {
                ...
                if (idofm > 0 && (c2 > 1e-14 || c2 < -1e-14)) {
                  insertas(&irowdd, &mast1, &idofs, &idofm, &ifree, &nzsdd, &c2,
                           &audd);
                }
        ...
        } else if ((idofm > 0) && (idofs <= 0) &&
                   (contribution > 1e-18 || contribution < -1e-18)) {
          // mpc on slave node

          for (jj = nslavmpc[2 * (isn - 1)]; jj < nslavmpc[2 * (isn - 1) + 1];
               jj++) {
            ist = islavmpc[jj];
            dirdep = nodempc[3 * (ist - 1) + 1];
            coefdep = coefmpc[ist - 1];
          ...
          for (jj = nslavmpc[2 * (imn - 1)]; jj < nslavmpc[2 * (imn - 1) + 1];
            ...
            if (ll == (dirdep - 1)) {
              while (index != 0) {
                ...
                c2 = -coefmpc[index - 1] * contribution / coefdep;
                idofm = nactdof[mt * (node1 - 1) + (dirind - 1) + 1];
                for (kk = nslavmpc[2 * (isn - 1)];
                     kk < nslavmpc[2 * (isn - 1) + 1]; kk++) {
                  ist2 = islavmpc[kk];
                  ...
                  if (ll == (dirdep2 - 1)) {
                    while (index2 != 0) {
                      ...
                      if (idofs > 0 && idofm > 0 &&
                          (c3 > 1e-14 || c3 < -1e-14)) {
                        insertas(&irowdd, &mast1, &idofs, &idofm, &ifree,
                                 &nzsdd, &c3, &audd);
                      }
    ...
    for (j = jqb[i] - 1; j < jqb[i + 1] - 1; j++) {
      ...
      for (ll = k; ll < 3; ll++) {
        ...
        if ((idofs > 0) && (idofm > 0) &&
          ...
          if (imn < 0) {
            for (jj = nmastmpc[2 * (-imn - 1)];
                 jj < nmastmpc[2 * (-imn - 1) + 1]; jj++) {
              ist = imastmpc[jj];
              dirdep = nodempc[3 * (ist - 1) + 1];
              coefdep = coefmpc[ist - 1];
              index = nodempc[3 * (ist - 1) + 2];
          ...
          for (jj = nslavmpc[2 * (isn - 1)]; jj < nslavmpc[2 * (isn - 1) + 1];
            ...
            if (ll == (dirdep - 1)) {
              while (index != 0) {
                ...
                idofs = nactdof[mt * (node1 - 1) + (dirind - 1) + 1];
                if (idofs > 0 && (c2 > 1e-14 || c2 < -1e-14)) {
                  insertas(&irowbd, &mast1, &idofs, &idofm, &ifree, nzsbd, &c2,
                           &aubd);
                }
                index = nodempc[3 * (index - 1) + 2];
              }
                  ...
                  for (kk = nslavmpc[2 * (isn - 1)];
                    ...
                    if (ll == (dirdep2 - 1)) {
                      while (index2 != 0) {
                        ...
                        if (idofs > 0 && idofm > 0 &&
                            (c3 > 1e-14 || c3 < -1e-14)) {
                          insertas(&irowbd, &mast1, &idofs, &idofm, &ifree,
                                   nzsbd, &c3, &aubd);
                        }
                        index2 = nodempc[3 * (index2 - 1) + 2];
                      }
    ...
    if (tieset[i * (81 * 3) + 80] == 'C') {
      for (j = nslavnode[i]; j < nslavnode[i + 1]; j++) {
        ...
        if (islavact[j] > -1) {
          for (ll = 0; ll < 3; ll++) {
            ...
            if (idofm > -1) {
              for (k = jqbdtil2[idofm] - 1; k < jqbdtil2[idofm + 1] - 1; k++) {
                ...
                } else if (irowbdtil2[k] ==
                           nactdof[mt * (nodesf - 1) + 1 + 1]) {
                  // dof2
                  dloc[9 * j + ll * 3 + 1] = aubdtil2[k];
                } else if (irowbdtil2[k] ==
                           nactdof[mt * (nodesf - 1) + 2 + 1]) {
                  // dof3
          ...
          dinvloc[9 * j + 0] = +1.0 / detdloc *
                               (dloc[9 * j + 4] * dloc[9 * j + 8] -
                                dloc[9 * j + 7] * dloc[9 * j + 5]);
          dinvloc[9 * j + 1] = -1.0 / detdloc *
                               (dloc[9 * j + 1] * dloc[9 * j + 8] -
                                dloc[9 * j + 7] * dloc[9 * j + 2]);
          dinvloc[9 * j + 2] = +1.0 / detdloc *
                               (dloc[9 * j + 1] * dloc[9 * j + 5] -
                                dloc[9 * j + 4] * dloc[9 * j + 2]);
          dinvloc[9 * j + 3] = -1.0 / detdloc *
                               (dloc[9 * j + 3] * dloc[9 * j + 8] -
                                dloc[9 * j + 6] * dloc[9 * j + 5]);
          dinvloc[9 * j + 4] = +1.0 / detdloc *
                               (dloc[9 * j + 0] * dloc[9 * j + 8] -
                                dloc[9 * j + 6] * dloc[9 * j + 2]);
          dinvloc[9 * j + 5] = -1.0 / detdloc *
                               (dloc[9 * j + 0] * dloc[9 * j + 5] -
                                dloc[9 * j + 3] * dloc[9 * j + 2]);
          dinvloc[9 * j + 6] = +1.0 / detdloc *
                               (dloc[9 * j + 3] * dloc[9 * j + 7] -
                                dloc[9 * j + 6] * dloc[9 * j + 4]);
          dinvloc[9 * j + 7] = -1.0 / detdloc *
                               (dloc[9 * j + 0] * dloc[9 * j + 7] -
                                dloc[9 * j + 6] * dloc[9 * j + 1]);
          dinvloc[9 * j + 8] = +1.0 / detdloc *
                               (dloc[9 * j + 0] * dloc[9 * j + 4] -
          ...
          for (ll = 0; ll < 3; ll++) {
            ...
            for (k = 0; k < 3; k++) {
              ...
              if (idofs > 0 && idofm > 0 &&
                  (contribution > 1.e-18 || contribution < -1.e-18)) {
    ...
    for (j = jqbdtil2[i] - 1; j < jqbdtil2[i + 1] - 1; j++) {
      ...
      if (islavactdof[idofm - 1] < 0 ||
        ...
        if (islavactdof[idofs - 1] < 1 && islavactdof[idofm - 1] > 0) {
          ...
          printf("bdfill: noder %" ITGFORMAT " %" ITGFORMAT " nodec %" ITGFORMAT
                 " %" ITGFORMAT " \n",
                 islavnode[(ITG)floor(islavactdof[idofs - 1] / 10.) - 1], idofs,
                 islavnode[(ITG)floor(islavactdof[idofm - 1] / 10.) - 1],
                 idofm);
          FORTRAN(stop, ());
          ...
          printf("bdfill: slavenode row   %" ITGFORMAT " %" ITGFORMAT
                 ...
                 islavnode[(ITG)floor(islavactdof[idofs - 1] / 10.) - 1], idofs,
                 imastnode[-((ITG)floor(islavactdof[idofm - 1] / 10.)) - 1],
                 idofm);
          FORTRAN(stop, ());
        } else if (islavactdof[idofs - 1] < 1 && islavactdof[idofm - 1] == 0) {
          printf("bdfill: something went wrong in aubdtil2! Stop!\n");
        ...
        if (jrow == 1) {
          ...
          if (j + 1 < jqbdtil2[i + 1] - 1 && irowbdtil2[j + 1] == idof3) {
            e3 = aubdtil2[j + 1];
            ++iadd;
          } else if (j + 2 < jqbdtil2[i + 1] - 1 &&
                     irowbdtil2[j + 2] == idof3) {
            e3 = aubdtil2[j + 2];
            ++iadd;
          } else {
            e3 = 0.0;
        ...
        if (idof1 > 0 && (contribution > 1.e-18 || contribution < -1.e-18)) {
          insertas_ws(&irowbdtil, &idof1, &idofm, &ifree, nzsbdtil,
                      &contribution, &aubdtil);
        }
        ...
        if (idof2 > 0 && (contribution > 1.e-18 || contribution < -1.e-18)) {
          insertas_ws(&irowbdtil, &idof2, &idofm, &ifree, nzsbdtil,
                      &contribution, &aubdtil);
        }
        ...
        if (idof3 > 0 && (contribution > 1.e-18 || contribution < -1.e-18)) {
          insertas_ws(&irowbdtil, &idof3, &idofm, &ifree, nzsbdtil,
                      &contribution, &aubdtil);
        }
        j = j + iadd;
      }
      ...
      for (j = jqbdtil2[i] - 1; j < jqbdtil2[i + 1] - 1; j++) {
        ...
        if (islavactdof[idofm - 1] < 0 ||
          ...
          if (islavactdof[idofs - 1] < 1 && islavactdof[idofm - 1] > 0) {
            printf("bdfill: something went wrong in aubdtil2! Stop!\n");
            printf(
                "bdfill: noder %" ITGFORMAT " %" ITGFORMAT " nodec %" ITGFORMAT
                " %" ITGFORMAT " \n",
                islavnode[(ITG)floor(islavactdof[idofs - 1] / 10.) - 1], idofs,
            ...
            printf("bdfill: slavenode row   %" ITGFORMAT " %" ITGFORMAT
                   " master node column %" ITGFORMAT " %" ITGFORMAT " \n",
                   islavnode[(ITG)floor(islavactdof[idofs - 1] / 10.) - 1],
                   idofs,
                   imastnode[-((ITG)floor(islavactdof[idofm - 1] / 10.)) - 1],
                   idofm);
            FORTRAN(stop, ());

          } else if (islavactdof[idofs - 1] < 1 &&
                     islavactdof[idofm - 1] == 0) {
            printf("bdfill: something went wrong in aubdtil2! Stop!\n");
            printf("bdfill: slavenode row   %" ITGFORMAT " %" ITGFORMAT
                   " master node column %" ITGFORMAT " \n",
          ...
          if (idof0 > 0 && (contribution > 1.e-18 || contribution < -1.e-18)) {
            insertas_ws(&irowbdtil, &idof0, &idofm, &ifree, nzsbdtil,
                        &contribution, &aubdtil);
          }
        }
      }
