...
void contactmortar(
  ...
  ITG i, j, k, ntrimax,
      *nx = NULL, *ny = NULL, *nz = NULL, nintpoint = 0, nzsbd, *irowbd = NULL,
      *irowdd = NULL, *irowddinv = NULL, *irowddtil = NULL, *irowbdtil = NULL,
      nzs2, *irowddtil2 = NULL, *irowbdtil2 = NULL, l, nstart, kflag, ntri, ii,
      regmode, derivmode, *irowc = NULL, *imastsurf = NULL, *irow = NULL,
      *irowb = NULL, *irowbhelp = NULL, *irowd = NULL, *irowdtil = NULL,
      *irowbtil = NULL, nacti, ninacti, nnogap, nstick, nnolm, nnoslav,
      nzsbdtil, nzsbdtil2;

  double *xo = NULL, *yo = NULL, *zo = NULL, *x = NULL, *y = NULL, *z = NULL,
         *aubd = NULL, *cstresstil = NULL, scal, *audd = NULL, *auddtil = NULL,
         *auddtil2 = NULL, *auddinv = NULL, *auc = NULL, *pmastsurf = NULL,
         *gapmints = NULL, *au = NULL, *pslavsurf = NULL, *aubdtil = NULL,
         *aubdtil2 = NULL, *Bd = NULL, *Bdhelp = NULL, *Dd = NULL,
         *Ddtil = NULL, *Bdtil = NULL, mu, fkninv, fktauinv, p0, beta,
         *rs = NULL, *rsb = NULL;
  ...
  irowbd = *irowbdp;
  aubdtil = *aubdtilp;
  irowbdtil = *irowbdtilp;
  aubdtil2 = *aubdtil2p;
  irowbdtil2 = *irowbdtil2p;
  irowdd = *irowddp;
  audd = *auddp;
  irowddtil = *irowddtilp;
  auddtil = *auddtilp;
  irowddtil2 = *irowddtil2p;
  auddtil2 = *auddtil2p;
  irowddinv = *irowddinvp;
  auddinv = *auddinvp;
  irowb = *irowbp;
  Bd = *Bdp;
  irowbhelp = *irowbhelpp;
  Bdhelp = *Bdhelpp;
  irowd = *irowdp;
  Dd = *Ddp;
  irowdtil = *irowdtilp;
  ...
  FORTRAN(genislavactdof, (ntie, tieset, nactdof, nslavnode, nmastnode,
                           imastnode, islavactdof, islavnode, mi, ithermal));
  ...
  if (*iit == 1) {
    ...
    if ((*iinc == 1) && (*iit == 1) && (*istep == 1)) {
      ...
      NNEW(ny, ITG, ntrimax);
      NNEW(nz, ITG, ntrimax);
      FORTRAN(genfirstactif, (tieset, ntie, itietri, cg, straight, co, vold, xo,
                              yo, zo, x, y, z, nx, ny, nz, istep, iinc, iit, mi,
                              imastop, nslavnode, islavnode, set, nset,
                              istartset, iendset, ialset, islavact, tietol));
    ...
    FORTRAN(nortanslav,
            (tieset, ntie, ipkon, kon, lakon, set, co, vold, nset, islavsurf,
             itiefac, islavnode, nslavnode, slavnor, slavtan, mi));
    ...
    for (i = 0; i < *ntie; i++) {
      ...
      if (tieset[i * (81 * 3) + 80] == 'C') {
        ...
        for (l = itiefac[2 * i]; l <= itiefac[2 * i + 1]; l++) {
          ...
          FORTRAN(slavintmortar,
                  (ntie, itietri, ipkon, kon, lakon, straight, &nintpoint,
                   koncont, co, vold, xo, yo, zo, x, y, z, nx, ny, nz, iinc,
                   islavsurf, imastsurf, pmastsurf, islavnode, nslavnode,
                   imastop, gapmints, islavact, mi, ncont, ipe, ime, pslavsurf,
    ...
    FORTRAN(gendualcoeffs,
            (tieset, ntie, ipkon, kon, lakon, co, vold, islavact, islavsurf,
             itiefac, islavnode, nslavnode, mi, pslavsurf, pslavdual));
    ...
    bdfill(&irowbd, jqbd, &aubd, &nzsbd, &irowbdtil, jqbdtil, &aubdtil,
           &nzsbdtil, &irowbdtil2, jqbdtil2, &aubdtil2, &nzsbdtil2, &irowdd,
           jqdd, &audd, &irowddtil, jqddtil, &auddtil, &irowddtil2, jqddtil2,
           &auddtil2, &irowddinv, jqddinv, &auddinv, irowt, jqt, aut, irowtinv,
           jqtinv, autinv, ntie, ipkon, kon, lakon, nslavnode, nmastnode,
           imastnode, islavnode, islavsurf, imastsurf, pmastsurf, itiefac,
           tieset, neq, nactdof, co, vold, iponoels, inoels, mi, gapmints, gap,
           pslavsurf, pslavdual, &nintpoint, slavnor, nk, nmpc, ipompc, nodempc,
           coefmpc, ikmpc, ilmpc, nslavmpc, islavmpc, nmastmpc, imastmpc, iit,
           iinc, islavactdof, islavact, islavnodeinv, &Bd, &irowb, jqb, &Bdhelp,
           &irowbhelp, jqbhelp, &Dd, &irowd, jqd, &Ddtil, &irowdtil, jqdtil,
  ...
  if (*iit == 1) {
    ...
    for (i = 0; i < *ntie; i++) {
      if (tieset[i * (81 * 3) + 80] == 'C') {
        for (j = nslavnode[i]; j < nslavnode[i + 1]; j++) {
          nodes = islavnode[j];
          for (jj = jqdtil[nodes - 1] - 1; jj < jqdtil[nodes - 1 + 1] - 1;
               jj++) {
      ...
      for (j = nslavnode[i]; j < nslavnode[i + 1]; j++) {
        ...
        if ((*iit == 1)) {
          ...
          xlnold = cstresstil[(j) * (mt) + 0] * slavnor[(j * 3) + 0] +
                   cstresstil[(j) * (mt) + 1] * slavnor[(j * 3) + 1] +
                   cstresstil[(j) * (mt) + 2] * slavnor[(j * 3) + 2];
          FORTRAN(getcontactparams, (&mu, &regmode, &fkninv, &fktauinv, &p0,
                                     &beta, tietol, elcon, &i, ncmat_, ntmat_));
          derivmode = 0;
          if (islavact[j] > -1) {
          ...
          FORTRAN(regularization_gn_c,
                  (&xlnold, &derivmode, &regmode, &gnc, &aninvloc, &p0, &beta,
                   elcon, nelcon, &i, ntmat_, plicon, nplicon, npmat_, ncmat_,
                   tietol, &scal));
          if (mu > 1.E-10) {
            bp_old[j] = mu * (xlnold);
  ...
  *nzsc = nzs[1];
  ...
  multimortar(
      &au, ad, &irow, jq, &nzs2, &auc, adc, &irowc, jqc, nzsc, aubd, irowbd,
      jqbd, aubdtil, irowbdtil, jqbdtil, aubdtil2, irowbdtil2, jqbdtil2, irowdd,
      jqdd, audd, irowddtil2, jqddtil2, auddtil2, irowddinv, jqddinv, auddinv,
      Bd, irowb, jqb, Dd, irowd, jqd, Ddtil, irowdtil, jqdtil, neq, b, bhat,
      islavnode, imastnode, nslavnode, nmastnode, islavact, islavactdof, gap,
      slavnor, slavtan, vold, vini, cstress, cstressini, bp_old, nactdof, ntie,
      mi, nk, nboun, ndirboun, nodeboun, xboun, nmpc, ipompc, nodempc, coefmpc,
      ikboun, ilboun, ikmpc, ilmpc, nslavspc, islavspc, nslavmpc, islavmpc,
      tieset, islavtie, nelcon, elcon, tietol, ncmat_, ntmat_, plicon, nplicon,
      npmat_, dtime, irowt, jqt, aut, irowtinv, jqtinv, autinv, islavnodeinv,
      &k, nmethod, bet, ithermal, plkcon, nplkcon);
  ...
  *irowbdp = irowbd;
  *aubdtilp = aubdtil;
  *irowbdtilp = irowbdtil;
  *aubdtil2p = aubdtil2;
  *irowbdtil2p = irowbdtil2;
  *auddp = audd;
  *irowddp = irowdd;
  *auddinvp = auddinv;
  *irowddinvp = irowddinv;
  *auddtilp = auddtil;
  *irowddtilp = irowddtil;
  *auddtil2p = auddtil2;
  *irowddtil2p = irowddtil2;
  *Bdp = Bd;
  *irowbp = irowb;
  *Bdhelpp = Bdhelp;
  *irowbhelpp = irowbhelp;
  *Ddp = Dd;
  *irowdp = irowd;
  *Ddtilp = Ddtil;
  *irowdtilp = irowdtil;
