...
void trafontmortar2(
  ...
  ITG i, j, jj, j2, k, l, idof1, idof2, idof3, iadd, jrow, islavnodeentry,
      mt = mi[1] + 1, nodes, node, derivmode, regmode, *irow_antil = NULL,
      *irow_amtil = NULL, *irow_aitil = NULL, *irow_aatil = NULL,
      *irow_amtil1 = NULL, *irow_amtil2 = NULL, *irow_aitil1 = NULL,
      *irow_aitil2 = NULL, *irow_aatil1 = NULL, *irow_aatil2 = NULL, nzs_antil,
      nzs_amtil, nzs_aitil, nzs_aatil, *jq_amtil1 = NULL, *jq_amtil2 = NULL,
      nzs_amtil1, nzs_amtil2, nzs_aitil1, nzs_aitil2, nzs_aatil1, nzs_aatil2,
      *jq_aitil1 = NULL, *jq_aitil2 = NULL, *jq_aatil1 = NULL,
      *jq_aatil2 = NULL, ifree_antil, ifree_amtil1, ifree_amtil2, ifree_aitil1,
      ifree_aitil2, ifree_aatil1, ifree_aatil2;

  double t1, t2, e1, e2, e3, contribution, hpn, scal, bp,
      constant = 1.E10, constantt = 1.E10, lambda_n, *u_tilde = NULL, resreg[2],
      *cstress2 = NULL, *cstressini2 = NULL, that[6], n11, n22, aninvloc, gnc,
      dgnc, dgnc1, mu, p0, beta_e, atauinvloc, lambda_t[2], lambdaini_t[2],
      lambdatilde_t[2], ltu[2], ltslip[6], rphat[2], n[3], n2[3], t[6],
      utildep_t[2], rslip[6], *au_antil = NULL, *au_amtil = NULL,
      *au_aitil = NULL, *au_aatil = NULL, *au_amtil1 = NULL, *au_amtil2 = NULL,
      *au_aitil1 = NULL, *au_aitil2 = NULL, *au_aatil1 = NULL,
      *au_aatil2 = NULL;
  ...
  au_aitil = *au_aitilp;
  au_aatil = *au_aatilp;
  irow_antil = *irow_antilp;
  irow_amtil = *irow_amtilp;
  irow_aitil = *irow_aitilp;
    ...
    if (tieset[i * (81 * 3) + 80] == 'C') {
      for (j = nslavnode[i]; j < nslavnode[i + 1]; j++) {
        ...
        for (jj = jqdtil[nodes - 1] - 1; jj < jqdtil[nodes - 1 + 1] - 1; jj++) {
          if (islavnodeinv[irowdtil[jj] - 1] > 0 &&
              islavnodeinv[nodes - 1] > 0) {
            for (l = 0; l < 3; l++) {
              ...
              cstressini2[(islavnodeinv[irowdtil[jj] - 1] - 1) * mt + l] +=
                  Ddtil[jj] *
                  cstressini[(islavnodeinv[nodes - 1] - 1) * mt + l];
            }
          } else {
  ...
  for (j = 0; j < *row_ln; j++) {
    ...
    for (i = jq_dan[j] - 1; i < jq_dan[j + 1] - 1; i++) {
      ...
      trafontspcmpc(n, t, n2, that, &islavnodeentry, nboun, ndirboun, nodeboun,
                    xboun, ipompc, nodempc, coefmpc, ikboun, ilboun, ikmpc,
                    ilmpc, nslavspc, islavspc, nslavmpc, islavmpc, &node);
      ...
      FORTRAN(getcontactparams,
              (&mu, &regmode, &aninvloc, &atauinvloc, &p0, &beta_e, tietol,
               elcon, &islavtie[islavnodeentry - 1], ncmat_, ntmat_));
      constantt = min(constant, 1.0 / atauinvloc);
      derivmode = 1;
      FORTRAN(regularization_gn_c,
              (&lambda_n, &derivmode, &regmode, &dgnc, &aninvloc, &p0, &beta_e,
               elcon, nelcon, &islavtie[islavnodeentry - 1], ntmat_, plicon,
               nplicon, npmat_, ncmat_, tietol, &scal));

      if (islavact[islavnodeentry - 1] == 1 ||
          islavact[islavnodeentry - 1] == 2) {
        derivmode = 1;

        /* perturbed lagrange,tangential direction
           (see phd-thesis Sitzmann,Chapter 3.4.2) */

        FORTRAN(regularization_slip_lin,
                (utildep_t, &bp, &atauinvloc, resreg, &derivmode, islavact,
                 lambda_t, lambdatilde_t, &constantt, &islavnodeentry, n2, t,
                 that, &mu, rslip, ltslip, ltu));
        rphat[0] = 0.0;
        rphat[1] = 0.0;
          ...
          if (idof1 > -1 && idof2 > -1 && idof3 > -1) {
            ...
            if (jrow == 1) {
              ...
              if (i + 1 < jq_bdtil2[j + 1] - 1 &&
                  a_flagr[irow_bdtil2[i + 1] - 1] - 1 == idof2) {
                e2 = au_bdtil2[i + 1];
                ++iadd;
              } else {
                e2 = 0.0;
  ...
  add_rect(au_amtil1, irow_amtil1, jq_amtil1, *row_la, *row_lm, au_amtil2,
           irow_amtil2, jq_amtil2, *row_la, *row_lm, &au_amtil, &irow_amtil,
           jq_amtil, &nzs_amtil);
  ...
  add_rect(au_aitil1, irow_aitil1, jq_aitil1, *row_la, *row_li, au_aitil2,
           irow_aitil2, jq_aitil2, *row_la, *row_li, &au_aitil, &irow_aitil,
           jq_aitil, &nzs_aitil);
  ...
  add_rect(au_aatil1, irow_aatil1, jq_aatil1, *row_la, *row_la, au_aatil2,
           irow_aatil2, jq_aatil2, *row_la, *row_la, &au_aatil, &irow_aatil,
           jq_aatil, &nzs_aatil);
  ...
  for (k = 0; k < *row_la; k++) {
    if (islavactdof[a_flagr[k] - 1] > 0) {
      ...
      FORTRAN(regularization_gn_c,
              (&lambda_n, &derivmode, &regmode, &gnc, &aninvloc, &p0, &beta_e,
               elcon, nelcon, &islavtie[islavnodeentry - 1], ntmat_, plicon,
               nplicon, npmat_, ncmat_, tietol, &scal));
      ...
      if (islavact[islavnodeentry - 1] == 1 ||
        ...
        if (jrow == 4 && ithermal[0] < 2) {
          ...
          if (idof1 > -1 && idof2 > -1 && idof3 > -1 && jrow == 1) {
            ...
            f_atil[k + 1] = (rslip[0] * e1 + rslip[1] * e2 + rslip[2] * e3) +
                            rphat[0] - ltu[0];
            f_atil[k + 2] = (rslip[3] * e1 + rslip[4] * e2 + rslip[5] * e3) +
                            rphat[1] - ltu[1];
            k = k + 2;
          } else if (idof2 > -1 && idof3 > -1) {
            // 2D auf 3D
  ...
  *au_aitilp = au_aitil;
  *au_aatilp = au_aatil;
  *irow_antilp = irow_antil;
  *irow_amtilp = irow_amtil;
  *irow_aitilp = irow_aitil;
