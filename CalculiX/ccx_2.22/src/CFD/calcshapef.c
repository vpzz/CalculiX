...
void calcshapef(ITG *nvar_, ITG *ipvar, double **varp, ITG *ne, char *lakon,
  ...
  ITG ifaceq[48] = {4, 3, 2, 1, 11, 10, 9,  12, 5, 6, 7, 8, 13, 14, 15, 16,
                    1, 2, 6, 5, 9,  18, 13, 17, 2, 3, 7, 6, 10, 19, 14, 18,
                    3, 4, 8, 7, 11, 20, 15, 19, 4, 1, 5, 8, 12, 17, 16, 20};

  ITG ifacet[24] = {1, 3, 2, 7, 6,  5, 1, 2, 4, 5, 9,  8,
                    2, 3, 4, 6, 10, 9, 1, 4, 3, 8, 10, 7};

  ITG ifacew[40] = {1, 3,  2,  9,  8, 7, 0, 0,  4,  5,  6, 10, 11, 12,
                    0, 0,  1,  2,  5, 4, 7, 14, 10, 13, 2, 3,  6,  5,
                    8, 15, 11, 14, 4, 6, 3, 1,  12, 15, 9, 13};

  double *var = NULL, xl[60], xi, et, ze, xsj, xl2[24], xs2[21], shp2[56],
         xsj2[3], xi3d, et3d, ze3d, weight, *varf = NULL, yyl[8], y;

  /* gauss points */

  double gauss2d1[2] = {0., 0.};

  double gauss2d2[8] = {-0.577350269189626, -0.577350269189626,
  ...
  double gauss3d7[6] = {0.333333333333333,  0.333333333333333,
      ...
      do {
        ...
        if (nope == 8) {
          for (j = 0; j < nopes; j++) {
            for (k = 0; k < 3; k++) {
              xl2[3 * j + k] =
                  co[3 * (kon[indexe + ifaceq[8 * (ig - 1) + j] - 1] - 1) + k];
            }
          }
        } else if (nope == 4) {
          for (j = 0; j < nopes; j++) {
            for (k = 0; k < 3; k++) {
              xl2[3 * j + k] =
                  co[3 * (kon[indexe + ifacet[6 * (ig - 1) + j] - 1] - 1) + k];
            }
          }
        } else {
          for (j = 0; j < nopes; j++) {
            for (k = 0; k < 3; k++) {
              xl2[3 * j + k] =
                  co[3 * (kon[indexe + ifacew[8 * (ig - 1) + j] - 1] - 1) + k];
            }
          }
        }
        ...
        for (kk = 0; kk < mint2d; kk++) {
          ...
          if (strcmp1(&lakon[8 * i + 3], "8R") == 0) {
            ...
            FORTRAN(shape8h,
                    (&xi3d, &et3d, &ze3d, xl, &xsj, &varf[nvarf], &iflag));
          } else if (strcmp1(&lakon[8 * i + 3], "8") == 0) {
