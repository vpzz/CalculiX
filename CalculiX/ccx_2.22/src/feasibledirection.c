...
void feasibledirection(
  ...
  char *objectset = NULL, cflag[1] = " ", description[13] = "            ",
       *lakon = NULL, *gradprojname = NULL, *lakonfa = NULL;

  ITG i, *ipoacti = NULL, *iconstacti = NULL, nactive = 0, nnlconst, iscaleflag,
         mode = -1, *inameacti = NULL, nconstraint = 0, *inum = NULL, iinc = 1,
         noddiam = -1, ngraph = 1, idesvar = 0, inorm = 0, irand = 0,
         ishape = 0, icoordinate = 1, *ipkon = NULL, iobject, istart,
         *kon = NULL, *ielmat = NULL, *nodedesiinv = NULL, ifeasd = 0, methodfd,
         igeoconst = 0, *nodedesipos = NULL, *ipoface = NULL, *nodface = NULL,
         *konfa = NULL, *ipkonfa = NULL, *iponoelfa = NULL, *inoelfa = NULL,
         nsurfs, ifreemax, inoelsize, iregion = 0, *nod2nd3rd = NULL,
         *nodedesiboun = NULL, addout, ipos, ifree, nfield, iforce,
         *inoel = NULL, *iponoel = NULL, *istartdesi = NULL, *ialdesi = NULL;

  double *objnorm = NULL, *dgdxglob = NULL, *stn = NULL, ptime = 0.,
         *gradproj = NULL, *extnorini = NULL, *extnor = NULL, *feasdir = NULL,
         *tinc;
  ...
  if ((igeoconst == 1) || (strcmp1(&objectset[86], "BOU") == 0)) {
    ...
    FORTRAN(normalsonsurface_se,
            (ipkon, kon, lakon, extnorini, coini, nk, ipoface, nodface, nactdof,
             mi, nodedesiinv, &iregion, iponoelfa, ndesi, nodedesi, nod2nd3rd,
             ikboun, nboun, ne2d));
    ...
    for (i = 0; i < *nobject; i++) {
      if (strcmp1(&objectset[i * 405 + 3], "MEMBERSIZE") == 0) {
        printf(
            " Computation of geometric constraint "
            "MAXMEMEBERSIZE/MINMEMBERSIZE\n\n");
        ...
        frd_sen(co, nk, stn, inum, nmethod, kode, filab, &ptime, nstate_, istep,
                ...
                &i, objectset, ntrans, inotr, trab, &idesvar, orname,
        ...
        printf(
            " Computation of geometric constraint MAXGROWTH/MAXSHRINKAGE\n\n");

        iobject = i + 1;
        FORTRAN(maxdesvardisp,
                (nobject, nk, set, nset, istartset, iendset, ialset, &iobject,
                 nodedesiinv, dgdxglob, objectset, xdesi, coini, co,
                 nodedesipos, ndesi, nodedesi, g0, extnorini));
  ...
  if (methodfd == 1) {
    printf(
        " Computation of feasible direction with Gradient Descent method\n\n");
    ...
    FORTRAN(constassembly,
            (nobject, objectset, g0, ndesi, dgdxglob, nk, nodedesi, gradproj,
             set, nset, nodedesiboun, istartset, iendset, ialset, nodedesiinv));

    /* assembly of feasible direction */

    printf(" Computation of feasible direction\n\n");

    FORTRAN(calcfeasibledirection_gd,
            (ndesi, nodedesi, dgdxglob, &nactive, nobject, nk, gradproj));

    if (addout == 1) {
      if (strcmp1(&filab[4], "I") == 0) {
        nfield = 3;
        iforce = 0;
        FORTRAN(map3dto1d2d, (gradproj, ipkon, inum, kon, lakon, &nfield, nk,
      ...
      frd_sen(co, nk, stn, inum, nmethod, kode, filab, &ptime, nstate_, istep,
              &iinc, &mode, &noddiam, description, mi, &ngraph, ne, cs, set,
              nset, istartset, iendset, ialset, jobnamec, output, gradproj,
              &ipos, gradprojname, ntrans, inotr, trab, &idesvar, orname,
              &icoordinate, &inorm, &irand, &ishape, &ifeasd);
    }
    ...
    printf(
        " Computation of feasible direction with Gradient Projection "
        "method\n\n");
    ...
    FORTRAN(checkconstraint,
            (nobject, objectset, g0, &nactive, &nnlconst, ipoacti, ndesi,
             dgdxglob, nk, nodedesi, iconstacti, objnorm, inameacti));
  ...
  filterforwardmain(co, gradproj, nk, nodedesi, ndesi, objectset, xdesi,
                    feasdir, ne, iponoelfa, inoelfa, lakonfa, konfa, ipkonfa,
                    nodedesiinv, istartdesi, ialdesi, ipkon, lakon, ipoface,
                    nodface, kon, &iregion, isolver, &nsurfs);
  ...
  if (strcmp1(&objectset[89], "E") == 0) {
    transitionmain(co, feasdir, nobject, nk, nodedesi, ndesi, objectset, ipkon,
                   kon, lakon, ipoface, nodface, nodedesiinv);
  }
  ...
  FORTRAN(writeinputdeck2, (feasdir, nodedesi, ndesi, inoel, iponoel, xdesi, co,
                            lakon, ipkon, kon, tinc, nk));
  ...
  frd_sen(co, nk, stn, inum, nmethod, kode, filab, &ptime, nstate_, istep,
          ...
          istartset, iendset, ialset, jobnamec, output, feasdir, &ipos,
          gradprojname, ntrans, inotr, trab, &idesvar, orname, &icoordinate,
