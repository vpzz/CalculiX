...
void checkinclength(double *time, double *ttime, double *theta, double *dtheta,
  ...
  if (*dtheta > *tmax) {
    *dtheta = *tmax;
    //	printf(" the increment size exceeds thetamax and is decreased to
    //%e\n\n",*dtheta**tper);
  }
  ...
  if ((*itpamp > 0) && (*idrct == 0)) {
    ...
    //	printf("istart=%" ITGFORMAT ",iend=%" ITGFORMAT ",inext=%" ITGFORMAT
    ...
    /* the check with *tmin
       was introduced to circumvent the following problem: if the new data point
       is smaller than the next data point, but the distance is very small, the
       next *dheta calculated a few lines below may be zero due to the
       subtraction of two nearly equal numbers; a zero *dtheta leads to a fatal
       error */
    ...
    if ((*inext < inew) || (ireduceincrement == 1)) {
      ...
      (*inext)++;
      *itp = 1;
      //	    printf(" the increment size exceeds a time point and is
      // decreased to %e\n\n",*dtheta**tper);
    } else {
      *itp = 0;
    }
