...
void getglobalresults(char *masterfile, ITG **integerglobp,
  ...
  ITG *kontet = NULL, *ifatet = NULL, *inodfa = NULL, *ipofa = NULL, type, n1,
      n2, n3, n4, *nnx = NULL, *nny = NULL, *nnz = NULL, *kon = NULL,
      *ipkon = NULL, *kontyp = NULL, *iparent = NULL, ifreefa = 1, kflag = 2,
      ne, netet, numnodes, nkon, indexe, istep, loadcase, nfaces, netet_,
      nktet = 0, nfield, j, nodes[4], i, read_mode = 0, nodenr,
      *integerglob = NULL, *ielemnr = NULL, istep_global, iparentel;

  ITG i1[24] = {3, 7, 8, 6, 4, 3, 8, 1, 3, 8, 5, 6,
                3, 5, 8, 1, 2, 3, 5, 6, 2, 5, 3, 1};
  ITG i2[12] = {1, 2, 3, 5, 1, 5, 3, 4, 4, 5, 3, 6};
  ITG i4[88] = {5,  20, 17, 13, 20, 8,  19, 16, 19, 7,  18, 15, 18, 6,  17,
                14, 1,  9,  12, 13, 12, 11, 4,  16, 11, 10, 3,  15, 9,  2,
                10, 14, 9,  13, 11, 12, 11, 13, 16, 12, 13, 17, 19, 20, 13,
                19, 16, 20, 17, 14, 19, 18, 19, 14, 15, 18, 9,  11, 14, 10,
                11, 15, 14, 10, 11, 19, 16, 13, 11, 15, 19, 14, 11, 14, 19,
                17, 11, 19, 13, 17, 11, 14, 17, 9,  11, 17, 13, 9};
  ITG i5[56] = {1,  7,  9,  10, 7,  2,  8,  11, 8,  3,  9,  12, 5,  13,
                14, 11, 13, 4,  15, 10, 14, 15, 6,  12, 11, 12, 7,  10,
                7,  12, 9,  10, 11, 7,  12, 8,  12, 7,  9,  8,  13, 15,
                11, 10, 11, 15, 12, 10, 13, 11, 15, 14, 15, 11, 12, 14};
  ITG i6[32] = {8, 9, 10, 4, 1, 5,  7, 8, 7, 6, 3, 10, 9, 8, 10, 7,
                8, 9, 5,  7, 9, 10, 6, 7, 5, 6, 7, 9,  5, 2, 6,  9};

  double *planfa = NULL, *cotet = NULL, *cgtet = NULL, *field = NULL, *x = NULL,
         *y = NULL, *z = NULL, *xo = NULL, *yo = NULL, *zo = NULL,
         *doubleglob = NULL;

  integerglob = *integerglobp;
  doubleglob = *doubleglobp;
  ...
  if ((anz[0].n == 0) || (anz[0].e == 0)) {
    printf(
        " *ERROR in getglobalresults: there are either no nodes or\n no "
        "elements or neither nodes nor elements in the master frd-file\n");
    FORTRAN(stop, ());
  }
  ...
  for (i = 0; i < ne; i++) {
    ...
    if (type == 1) {
      ...
      for (j = 0; j < 6; j++) {
        ...
        netet++;
        FORTRAN(createtet, (kontet, ifatet, &netet, inodfa, &ifreefa, planfa,
                            ipofa, nodes, cotet, &iparentel));
      }
    } else if (type == 2) {
      /* C3D6 */
  ...
  for (i = 0; i < netet; i++) {
    ...
    n4 = kontet[4 * i + 3] - 1;
    cgtet[3 * i] =
        (cotet[3 * n1] + cotet[3 * n2] + cotet[3 * n3] + cotet[3 * n4]) / 4.;
    cgtet[3 * i + 1] = (cotet[3 * n1 + 1] + cotet[3 * n2 + 1] +
                        cotet[3 * n3 + 1] + cotet[3 * n4 + 1]) /
                       4.;
    cgtet[3 * i + 2] = (cotet[3 * n1 + 2] + cotet[3 * n2 + 2] +
                        cotet[3 * n3 + 2] + cotet[3 * n4 + 2]) /
                       4.;
  }
  ...
  for (i = 0; i < anz[0].l; i++) {
    ...
    if ((istep_global == istep) && (strcmp1(lcase[i].name, "NDTEMP") == 0)) {
      loadcase = i;
      if (*iglob < 0)
        *sigma = (6.283185307 * (double)lcase[i].value) *
                 (6.283185307 * (double)lcase[i].value);
  ...
  if (loadcase > -1) {
    ...
    if (!read_mode && readfrdblock(loadcase, anz, node, lcase) == -1) {
      printf(
          " *ERROR in getglobalresults: Could not read data for "
          "Dataset:%" ITGFORMAT "\n",
          i + 1);
      FORTRAN(stop, ());
    }
    ...
    printf(
        " *INFO in getglobalresults: no temperature data\n was found for step "
        "%d in the global model\n\n",
        istep);
  }
  ...
  for (i = 0; i < anz[0].l; i++) {
    ...
    if ((istep_global == istep) && (strcmp1(lcase[i].name, "DISPR") != 0) &&
  ...
  if (loadcase > -1) {
    ...
    printf(
        " *INFO in getglobalresults: no displacement data\n was found for step "
        "%d in the global model\n\n",
        istep);
  }
  ...
  for (i = 0; i < anz[0].l; i++) {
    ...
    if ((istep_global == istep) && (strcmp1(lcase[i].name, "STRESS") == 0)) {
      loadcase = i;
      if (*iglob < 0)
        *sigma = (6.283185307 * (double)lcase[i].value) *
                 (6.283185307 * (double)lcase[i].value);
  ...
  if (loadcase > -1) {
    ...
    printf(
        " *INFO in getglobalresults: no stress data\n was found for step %d in "
        "the global model\n\n",
        istep);
  }
  ...
  for (i = 0; i < anz[0].l; i++) {
    ...
    if ((istep_global == istep) && (strcmp1(lcase[i].name, "FORC") == 0)) {
      loadcase = i;
      if (*iglob < 0)
        *sigma = (6.283185307 * (double)lcase[i].value) *
                 (6.283185307 * (double)lcase[i].value);
  ...
  if (loadcase > -1) {
    ...
    printf(
        " *INFO in getglobalresults: no force data\n was found for step %d in "
        "the global model\n\n",
        istep);
  }
  ...
  memcpy(&integerglob[nkon + 2 * ne + 7 * netet + 5], &iparent[0],
         sizeof(ITG) * netet);
  memcpy(&integerglob[nkon + 2 * ne + 8 * netet + 5], &ielemnr[0],
         sizeof(ITG) * ne);
  ...
  memcpy(&doubleglob[4 * nfaces + 6 * netet], &field[0],
         sizeof(double) * 13 * nktet);
  memcpy(&doubleglob[13 * nktet + 4 * nfaces + 6 * netet], &cotet[0],
  ...
  *integerglobp = integerglob;
  *doubleglobp = doubleglob;
