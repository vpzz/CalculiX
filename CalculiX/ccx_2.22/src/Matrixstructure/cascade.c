...
void cascade(ITG *ipompc, double **coefmpcp, ITG **nodempcp, ITG *nmpc,
  ...
  ITG i, j, index, id, idof, nterm, idepend,
      *nodempc = NULL, iexpand, ichange, indexold, mpc, indexnew, index1,
      index2, index1old, index2old, *jmpc = NULL, nl;
  ...
  for (i = 0; i < *nmpc; i++) {
    ...
    if ((strcmp1(&labmpc[20 * i], "                    ") == 0) ||
        ...
        (strcmp1(&labmpc[20 * i], "THERMALPRET") == 0) || (*iperturb < 2))
      jmpc[i] = 0;
        ...
        if ((id > 0) && (ikmpc[id - 1] == idof)) {
          ...
          do {
            if (fabs(coefmpc[index1 - 1]) > 0.) {
              ...
              if (fabs(coefmpc[index1 - 1]) < coefmin)
                coefmin = fabs(coefmpc[index1 - 1]);
            }
            index2old = index1;
          ...
          if (fabs(coefmpc[index1 - 1]) < 1.e-10) {
            ...
            printf("          dependent node: %" ITGFORMAT "",
                   nodempc[3 * index1 - 3]);
                ...
                *mpcfree = nodempc[3 * *mpcfree - 1];
                if (*mpcfree == 0) {
                  ...
                  *memmpc_ = (ITG)(1.1 * *memmpc_);
                  printf(
                      " *INFO in cascade: reallocating nodempc; new size = "
                      "%" ITGFORMAT "\n\n",
                      *memmpc_);
