...
void mastructmm(ITG *icol, ITG *jq, ITG **mastp, ITG **irowp, ITG *ipointer,
  ...
  ITG i, j, jj, k, index, idof2, idof1, nmast, ifree, kflag, isize,
      *mast = NULL, *irow = NULL, *next = NULL, jstart, inode1, inode2, istart,
      nope, iface, nodeface[9], indexe, inode2pos;
  ...
  for (k = 0; k < *ndesi; k++) {
    ...
    do {
      ...
      if (strcmp1(&lakonfa[8 * (iface - 1) + 1], "4") == 0) {
        nope = 4;
      } else if (strcmp1(&lakonfa[8 * (iface - 1) + 1], "8") == 0) {
        nope = 8;
      } else if (strcmp1(&lakonfa[8 * (iface - 1) + 1], "3") == 0) {
        nope = 3;
      } else if (strcmp1(&lakonfa[8 * (iface - 1) + 1], "6") == 0) {
        nope = 6;
      }
