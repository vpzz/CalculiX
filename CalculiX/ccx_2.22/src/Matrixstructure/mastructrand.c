...
void mastructrand(ITG *icols, ITG *jqs, ITG **mast1p, ITG **irowsp,
                  ITG *ipointer, ITG *nzss, ITG *ndesi, double *corrlen,
  ...
  ITG i, j, ii, index, jdof2, jdof1, nmast, ifree, kflag, isize,
      *mast1 = NULL, *irows = NULL, *next = NULL, jstart, idesvar,
      *neighbor = NULL, nnodesinside;
  ...
  for (idesvar = 0; idesvar < *ndesi; idesvar++) {
    ...
    FORTRAN(near3d_se,
            (xo, yo, zo, x, y, z, nx, ny, nz, &xo[idesvar], &yo[idesvar],
             &zo[idesvar], ndesi, neighbor, r, &nnodesinside, &searchlen));
