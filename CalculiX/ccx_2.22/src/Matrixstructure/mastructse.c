...
void mastructse(ITG *kon, ITG *ipkon, char *lakon, ITG *ne, ITG *ipompc,
  ...
  ITG i, j, k, id, index, jdof1, idof1, idof2, nmast, ifree, kdof1, node, ist,
      kflag, indexe, nope, isize, *mast1 = NULL, ii, *irows = NULL,
                                  mt = mi[1] + 1, *next = NULL, jstart, idesvar;
    ...
    for (ii = istartdesi[idesvar] - 1; ii < istartdesi[idesvar + 1] - 1; ii++) {
      ...
      for (j = 0; j < nope; ++j) {
        ...
        for (k = 1; k < 4; k++) {
          ...
          if (idof1 > 0) {
            insertfreq(ipointer, &mast1, &next, &idof1, &idof2, &ifree, nzss);
          } else if (*nmpc != 0) {
            if (idof1 != 2 * (idof1 / 2)) {
              id = (-idof1 + 1) / 2 - 1;
              ist = ipompc[id] - 1;
              ...
              do {
                jdof1 = nactdof[mt * (nodempc[3 * index - 3] - 1) +
                                nodempc[3 * index - 2]];
                if (jdof1 > 0) {
                  insertfreq(ipointer, &mast1, &next, &jdof1, &idof2, &ifree,
                             nzss);
                }
  ...
  for (i = 0; i < *ndesi; ++i) {
    if (jqs[i + 1] - jqs[i] > 0) {
      isize = jqs[i + 1] - jqs[i];
      FORTRAN(isortii,
              (&irows[jqs[i] - 1], &mast1[jqs[i] - 1], &isize, &kflag));
    }
  }
