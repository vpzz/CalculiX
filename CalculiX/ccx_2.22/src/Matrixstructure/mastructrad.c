...
void mastructrad(ITG *ntr, ITG *nloadtr, char *sideload, ITG *ipointerrad,
  ...
  ITG three = 3, i, j, k, l, ii, jj, icav, jcav, *mast1rad = NULL,
      *irowrad = NULL, ifree, nzsrad_, nmast, isubtract, isize, kflag, istart,
      istartold;
  ...
  for (ii = 1; ii <= *ntr; ii++) {
    ...
    for (jj = 1; jj <= ii; jj++) {
      ...
      jcav = atoi(crcav);
      if (icav == jcav) {
        insertrad(ipointerrad, &mast1rad, &irowrad, &ii, &jj, &ifree, &nzsrad_);
      }
    }
  }
  ...
  k = 0;
  for (i = 0; i < nmast; ++i) {
    if (mast1rad[i] == irowrad[i]) {
      ++isubtract;
    } else {
      mast1rad[i - isubtract] = mast1rad[i];
      irowrad[i - isubtract] = irowrad[i];
      if (k != mast1rad[i]) {
        for (l = k; l < mast1rad[i]; ++l) {
          jqrad[l] = i + 1 - isubtract;
        }
        k = mast1rad[i];
      }
      ++icolrad[k - 1];
  ...
  for (i = 0; i < *ntr; ++i) {
    if (jqrad[i + 1] - jqrad[i] > 0) {
      ...
      FORTRAN(isortii, (&irowrad[jqrad[i] - 1], &mast1rad[jqrad[i] - 1], &isize,
                        &kflag));
    }
  }
