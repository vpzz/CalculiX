...
void mastruct(ITG *nk, ITG *kon, ITG *ipkon, char *lakon, ITG *ne,
  ...
  ITG i, j, k, l, jj, ll, id, index, jdof1, jdof2, idof1, idof2, mpc1, mpc2,
      id1, id2, ist1, ist2, node1, node2, nmast, ifree, index1, index2, m, node,
      nzs_, ist, kflag, indexe, nope,
      *mast1 = NULL, *irow = NULL, icolumn, nmastboun, mt = mi[1] + 1,
      *next = NULL, nopeold = 0, indexeold, identical, jstart,
      iatleastonenonzero, idof, ndof, *ithread = NULL;
  ...
  if ((*iit < 0) || (*icascade != 0)) {
    ...
    if ((*ithermal < 2) || (*ithermal >= 3)) {
      for (i = 0; i < *ne; ++i) {
        ...
        /* Bernhardi start */
        else if (strcmp1(&lakon[8 * i + 3], "8I") == 0) {
          nope = 11;
          ndof = 3;
        } else if (strcmp1(&lakon[8 * i + 3], "20") == 0) {
          nope = 20;
          ndof = 3;
        }
        /* Bernhardi end */
    ...
    for (i = 0; i < *nmpc; ++i) {
      ...
      nactdof[mt * (nodempc[3 * index] - 1) + nodempc[3 * index + 1]] =
          -2 * i - 1;
    }
          ...
          if ((jdof1 > 0) && (jdof2 > 0)) {
            ...
            if (*nmpc > 0) {
              if (idof2 != 2 * (idof2 / 2)) {
                ...
                if (index == 0) continue;
                while (1) {
                  idof2 = nactdof[mt * (nodempc[3 * index - 3] - 1) +
                                  nodempc[3 * index - 2]];
                  if (idof2 > 0) {
                    insert(ipointer, &mast1, &next, &idof1, &idof2, &ifree,
                           &nzs_);
                  }
            ...
            if ((*nmethod == 2) || ((*nmethod == 4) && (*iperturb <= 1)) ||
              ...
              insertfreq(ipointer, &mast1, &next, &idof1, &icolumn, &ifree,
                         &nzs_);
            }
          }
            ...
            if ((mpc1 == 1) && (mpc2 == 1)) {
              ...
              if (id1 == id2) {
                ...
                while (1) {
                  idof1 = nactdof[mt * (nodempc[3 * index1 - 3] - 1) +
                                  nodempc[3 * index1 - 2]];
                  index2 = index1;
                  while (1) {
                    idof2 = nactdof[mt * (nodempc[3 * index2 - 3] - 1) +
                                    nodempc[3 * index2 - 2]];
                    if ((idof1 > 0) && (idof2 > 0)) {
                      insert(ipointer, &mast1, &next, &idof1, &idof2, &ifree,
                             &nzs_);
  ...
  if ((*nmethod == 2) || ((*nmethod == 4) && (*iperturb <= 1)) ||
    ...
    printf(
        " Using up to %d cpu(s) for setting up the structure of the matrix.\n",
        num_cpus);
...
void *sortingmt(ITG *i) {
  ...
  for (j = neqa; j < neqb; ++j) {
    if (jq1[j + 1] - jq1[j] > 0) {
      isize = jq1[j + 1] - jq1[j];
      FORTRAN(isortii,
              (&irow1[jq1[j] - 1], &mast11[jq1[j] - 1], &isize, &kflag1));
    }
  }
