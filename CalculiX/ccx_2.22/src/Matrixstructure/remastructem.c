...
void remastructem(ITG *ipompc, double **coefmpcp, ITG **nodempcp, ITG *nmpc,
  ...
  ITG *nodempc = NULL, *mast1 = NULL, *ipointer = NULL, mpcend, callfrommain, i,
      *irow = NULL, mt;
  ...
  callfrommain = 0;
  cascade(ipompc, &coefmpc, &nodempc, nmpc, mpcfree, nodeboun, ndirboun, nboun,
          ikmpc, ilmpc, ikboun, ilboun, &mpcend, labmpc, nk, memmpc_, icascade,
          maxlenmpc, &callfrommain, iperturb, ithermal);
  ...
  mastructem(nk, kon, ipkon, lakon, ne, nodeboun, ndirboun, nboun, ipompc,
             ...
             ikmpc, ilmpc, ipointer, nzs, ithermal, mi, ielmat, elcon, ncmat_,
             ntmat_, inomat, network);
  ...
  *irowp = irow;
  ...
  for (i = 0; i < neq[1]; i++) f[i] = 0.;
  RENEW(fext, double, neq[1]);
  for (i = 0; i < neq[1]; i++) fext[i] = 0.;
  RENEW(b, double, neq[1]);
  for (i = 0; i < neq[1]; i++) b[i] = 0.;
  RENEW(fini, double, neq[1]);
  for (i = 0; i < neq[1]; i++) fini[i] = 0.;

  if ((*nmethod == 2) || (*nmethod == 4)) {
    RENEW(aux2, double, neq[1]);
    for (i = 0; i < neq[1]; i++) aux2[i] = 0.;
