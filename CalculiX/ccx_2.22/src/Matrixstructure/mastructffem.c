...
void mastructffem(ITG *nk, ITG *kon, ITG *ipkon, char *lakon, ITG *ne,
  ...
  ITG i, j, k, l, jj, ll, id, index, jdof1, jdof2, idof1, idof2, mpc1, mpc2,
      ...
      istartold, idir, index1, index2, node, nzs_, ist, kflag, indexe, nope,
      isize, *mast1 = NULL, *irowv = NULL, *irowp = NULL, imaterial;
  ...
  *neqp = 0;
  ...
  /* generic mass (temperature/velocity/compressible pressure/turbulent)
   * entries*/
  ...
  if (*nk == 0) {
    printf(
        "\n *WARNING in mastructffem: no degrees of freedom in the generic "
        "mass matrix\n\n");
  }
  ...
  */
  ...
  for (i = 0; i < *nk; ++i) {
    if (jqv[i + 1] - jqv[i] > 0) {
      isize = jqv[i + 1] - jqv[i];
      FORTRAN(isortii,
              (&irowv[jqv[i] - 1], &mast1[jqv[i] - 1], &isize, &kflag));
    }
  }
          ...
          if (*nmpc > 0) {
            if (idof2 != 2 * (idof2 / 2)) {
              ...
              while (1) {
                ...
                if (idof2 > 0) {
                  insertcbs(ipointer, &mast1, &irowp, &idof1, &idof2, &ifree,
                            &nzs_);
                }
  ...
  if (*neqp == 0) {
    printf(
        "\n*WARNING in matructf: no degrees of freedom in the pressure "
        "matrix\n\n");
  }
  ...
  */
  ...
  for (i = 0; i < *neqp; ++i) {
    if (jqp[i + 1] - jqp[i] > 0) {
      isize = jqp[i + 1] - jqp[i];
      FORTRAN(isortii,
              (&irowp[jqp[i] - 1], &mast1[jqp[i] - 1], &isize, &kflag));
    }
  }
