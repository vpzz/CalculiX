...
void matrixstorage(double *ad, double **aup, double *adb, double *aub,
  ...
  ITG i, j, l, *irow = NULL, *ai = NULL, *aj = NULL, kflag = 2, ndim, jref,
               kstart, klen, npoint_, npoint, neq3, index, i3l, i3c, i3lo, i3co,
               idof, n, il, ic, id, itrans, ndim2, *ipoindex = NULL,
               mt = mi[1] + 1, *nactdofinv = NULL, *nodorig = NULL, inode, idir;
  ...
  if (*nmethod != 1) {
    ...
    if ((itrans == 0) || (mei[0] == 1)) {
      ...
      for (i = 0; i < *neq; i++) {
        ...
        klen = k - kstart;
        FORTRAN(isortiid,
                (&ai[kstart], &aj[kstart], &aa[kstart], &klen, &kflag));
      }
  ...
  *irowp = irow;
  ...
  if ((itrans == 0) || (mei[0] == 1)) {
    printf(
        " Storing the node and global direction per entry (row or column)\n in "
        "the stiffness and mass matrices in the form node.direction in file %s "
        "\n\n",
        fdof);
  } else {
    printf(
        " Storing the node and local direction per entry (row or column)\n in "
        "the stiffness and mass matrices in the form node.direction in file %s "
        "\n\n",
        fdof);
  }
