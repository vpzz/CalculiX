...
void mastructcs(ITG *nk, ITG *kon, ITG *ipkon, char *lakon, ITG *ne,
  ...
  ITG i, j, k, l, jj, ll, id, index, jdof1, jdof2, idof1, idof2, mpc1, mpc2,
      id1, id2, ist1, ist2, node1, node2, isubtract, nmast, ifree, istart,
      istartold, index1, index2, m, node, nzs_, ist, kflag, indexe, nope, isize,
      *mast1 = NULL, *irow = NULL, inode, icomplex, inode1, icomplex1, inode2,
      *next = NULL, icomplex2, kdof1, kdof2, ilength, lprev, ij, mt = mi[1] + 1,
      jstart;
  ...
  for (i = 0; i < *ne; ++i) {
    ...
    if (nope == -1) {
      ...
      mastructreadcs(ipompc, nodempc, nmpc, nactdof, jq, &mast1, neq, ipointer,
                     &nzs_, nmethod, mi, &next, &ifree, &i, ielmat, matname,
                     labmpc, mcs, cs, ics);
            ...
            if (idof2 != 2 * (idof2 / 2)) {
              ...
              while (1) {
                ...
                if (idof2 > 0) {
                  ...
                  kdof1 = idof1 + neq[0];
                  kdof2 = idof2 + neq[0];
                  insert(ipointer, &mast1, &next, &kdof1, &kdof2, &ifree,
                         &nzs_);
                  if ((icomplex != 0) && (idof1 != idof2)) {
                    insert(ipointer, &mast1, &next, &kdof1, &idof2, &ifree,
                           &nzs_);
                    insert(ipointer, &mast1, &next, &idof1, &kdof2, &ifree,
                           &nzs_);
                  }
                }
            ...
            if (id1 == id2) {
              ...
              while (1) {
                ...
                while (1) {
                  ...
                  } else if (strcmp1(&labmpc[(id1 - 1) * 20], "SUBCYCLIC") ==
                             0) {
                    for (ij = 0; ij < *mcs; ij++) {
                      ilength = cs[17 * ij + 3];
                      lprev = cs[17 * ij + 13];
              ...
              while (1) {
                ...
                while (1) {
                  ...
                  } else if (strcmp1(&labmpc[(id2 - 1) * 20], "SUBCYCLIC") ==
                             0) {
                    for (ij = 0; ij < *mcs; ij++) {
                      ilength = cs[17 * ij + 3];
                      lprev = cs[17 * ij + 13];
